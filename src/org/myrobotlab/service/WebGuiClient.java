...
public class WebGuiClient extends Service {
	...
	public WebGuiClient(String n) {
		...
		// https://github.com/Atmosphere/wasync/wiki/Getting-Started-with-wAsync
		// client = (AtmosphereClient) ClientFactory.getDefault().newClient();
		// client = (AtmosphereClient)
		// ClientFactory.getDefault().newClient(AtmosphereClient.class);
		
		// https://groups.google.com/forum/#!topic/atmosphere-framework/L1Je-jDUCaE
		/*
	...
	public void connect(String url) throws IOException, URISyntaxException {
	  ...
	  // TODO - security needs to be done here .. 
		 ...
		 * .transport(Request.TRANSPORT.WEBSOCKET).transport(Request.TRANSPORT.
		 * SSE) .transport(Request.TRANSPORT.LONG_POLLING);
		 */
		...
		// AsyncHttpClientConfig c = new AsyncHttpClientConfig().setAsyncHttpClientProviderConfig(config).build();
		AsyncHttpClient ahc = new AsyncHttpClient(new NettyAsyncHttpProvider(config));
		// HERE ===> Socket socket = client.create(client.newOptionsBuilder().runtime(ahc).build());
