  ...
  public Object process(String line) throws IOException {
    ...
    if (line.startsWith(cd)) {
      ...
      try {
        ...
        /*
         * Old Way Message msg = Encoder.decodePathInfo(path); if (msg != null)
         * { info("incoming msg[%s]", msg);
         * 
         * // get service - is this a security breech ? ServiceInterface si =
         * Runtime.getService(msg.name); Object ret = si.invoke(msg.method,
         * msg.data);
         * 
         * // want message ? or just data ? // configurable ... // if you data
         * with tags - you might as well do // message ! // - return only
         * callbacks this way -> // si.in(msg); if (ret != null && ret
         * instanceof Serializable) { // configurable use log or system.out ? //
         * FIXME - make getInstance configurable // Encoder // reference !!!
