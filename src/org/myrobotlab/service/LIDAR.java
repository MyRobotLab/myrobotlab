...
public class LIDAR extends Service implements SerialDataListener {
	...
	public void setBaud(int baudRate) throws Exception {
		...
		/*
		 * 9600 is default, but just in case you ever need it... PC sends : 02
		 * 00 02 00 20 42 52 08 LMS replies: 06 02 81 03 00 A0 00 10 36 1A
		 * (success)
		 */
		if (baudRate == 9600) {
			serial.write(new byte[] { 0x02, 0x00, 0x02, 0x00, 0x20, 0x42, 0x52, 0x08 });
		} /*
		 * 19200 PC sends : 02 00 02 00 20 41 51 08 LMS replies: 06 02 81 03 00
		 * A0 00 10 36 1A (success)
		 */else if (baudRate == 19200) {
			serial.write(new byte[] { 0x02, 0x00, 0x02, 0x00, 0x20, 0x41, 0x52, 0x08 });
		} /*
		 * 38400 PC sends : 02 00 02 00 20 41 51 08 LMS replies: 06 02 81 03 00
		 * A0 00 10 36 1A (success)
