...
public class Adafruit16CServoDriver extends Service implements ArduinoShield, ServoController {
  ...
  /*
   * public static final int pin = 3; // pin number public static final int left
   * = 300; // left position public static final int right = 450; // right
   * position
   */
  ...
  public boolean attach(Arduino arduino) {
    ...
    // set the program
    /*
     * Sketch sketch = new Sketch("Adafruit16CServoDriver",
     * newProgram.toString()); arduino.setSketch(sketch); // broadcast the
     * arduino state - ArduinoGUI should subscribe to // setProgram
     * broadcastState(); // state has changed let everyone know
     * arduino.broadcastState(); // servo9.attach(arduinoName, 9); // FIXME ???
     * - createServo(Integer i) // servo10.attach(arduinoName, 10);
     * 
