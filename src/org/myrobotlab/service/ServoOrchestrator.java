		...
		try {
			...
			for (ServoOrchestratorGUI_middlemiddle_panel[] p1 : sogui_ref.middlemiddle_ref.panels) {
				for (ServoOrchestratorGUI_middlemiddle_panel p : p1) {
					if (p != null) {
						bw.write(p.type + "~" + p.id + "~" + p.channel_name.getText() + "~" + p.servo_start.getText() + "~" + p.servo_channelid.getText() + "~"
								+ p.servo_goal.getText() + "~" + p.servo_min.getText() + "~" + p.servo_max.getText());
					} else {
						bw.write("null");
					}
					bw.newLine();
