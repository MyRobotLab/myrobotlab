...
public class Serial extends Service implements PortSource, QueueSource, SerialDataListener {
   ...
   * non-blocking mode (return immediately on read) timeout = x: set timeout to
   * x milliseconds
   */
   ...
   * be part os a SerialFilePlayer implementation
   * 
   * @param name
   */
  ...
  /**
   * decode relies on the rx codec decode method which will block a thread until
   * the out rx stream data buffer decodes a message which can be returned
   * 
   * @return
   */
   ...
   * blocking & timeout InputStream like interface - but regrettably InputStream
   * IS NOT A F#(@!! INTERFACE !!!!
   * 
   ...
   * @param timeoutMS
   *          - the amount of time to wait blocking until we return. 0 ms means
   *          the reading thread will potentially block forever.
  ...
  /**
   * uses key ascii, decimal, hex, arduino ... to dynamically set file formatter
   * 
   * @param key
   * @throws ClassNotFoundException
