...
public class Arduino extends Service implements Microcontroller, PinArrayControl, I2CBusController, I2CController, SerialDataListener, ServoController, MotorController,
  ...
  public Arduino(String n) {
    super(n);
    // super calls load() - which will load any variables which were serialized
    // and are not new'd and assigned in definition
    // if the boardInfo wasn't saved & loaded (sticky) - we create a new one
    /* removed now with the non-complexity of boardTypes
    if (boardInfo == null) {
  ...
  public List<PinDefinition> getPinList() {
    ...
    if ((pinIndex != null && board.contains("mega") && pinIndex.size() == 70) || (pinIndex != null && pinIndex.size() == 20)){
      return pinIndex.entrySet().stream()
          .map(x -> x.getValue())
  ...
  public boolean setArduinoPath(String path) {
    ...
    arduinoPath = path;
    ArduinoUtils.arduinoPath = arduinoPath; // THIS IS SILLY AND NOT NORMALIZED !
    save();
    return true;
  }
