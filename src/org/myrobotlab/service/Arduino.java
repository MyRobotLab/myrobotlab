...
public class Arduino extends Service implements Microcontroller, PinArrayControl, I2CBusController, I2CController,
	...
	/**
	 * class which contains and manages all pin definitions indexed by address
	 * or pin name
	 */
	...
	public Arduino(String n) {
		...
		/*
		 * removed now with the non-complexity of boardTypes if (boardInfo ==
		 * null) { boardInfo = new BoardInfo(); }
		 */
	...
	public Integer onByte(Integer newByte) {
		try {
			...
			if (byteCount == 2 + msgSize) {
				 ...
				 * *****"); ackRecievedLock.wait(10000); log.info("***** waited
				 * {} ms *****", (System.currentTimeMillis() - ts)); } catch
				 * (InterruptedException e) {// don't care} }
				 * 
				 * if (!ackRecievedLock.acknowledged) { log.error( "Ack not
				 * received : {} {}", Msg.methodToString(ioCmd[0]), numAck); } }
				 * }
				 ***/
