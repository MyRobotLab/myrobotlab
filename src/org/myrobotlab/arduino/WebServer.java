 ...
 * <p/>
 * This is a very simple, multi-threaded HTTP server, originally based on <a
 * href="http://j.mp/6BQwpI">this</a> article on java.sun.com.
 */
public class WebServer implements HttpConstants {
	...
	/*
	 * static void loadProps() throws IOException { File f = new File
	 * (System.getProperty("java.home")+File.separator+
	 * "lib"+File.separator+"www-server.properties"); if (f.exists()) {
	 * InputStream is =new BufferedInputStream(new FileInputStream(f));
	 * props.load(is); is.close(); String r = props.getProperty("root"); if (r
	 * != null) { root = new File(r); if (!root.exists()) { throw new Error(root
	 * + " doesn't exist as server root"); } } r = props.getProperty("timeout");
	 * if (r != null) { timeout = Integer.parseInt(r); } r =
	 * props.getProperty("workers"); if (r != null) { workers =
	 * Integer.parseInt(r); } r = props.getProperty("log"); if (r != null) {
	 * p("opening log file: " + r); log = new PrintStream(new
	 * BufferedOutputStream( new FileOutputStream(r))); } }
	 * 
	 * // if no properties were specified, choose defaults if (root == null) {
	 * root = new File(System.getProperty("user.dir")); } if (timeout <= 1000) {
	 * timeout = 5000; } if (workers < 25) { workers = 5; } if (log == null) {
	 * p("logging to stdout"); log = System.out; } }
	 * 
	 * static void printProps() { p("root="+root); p("timeout="+timeout);
	 * p("workers="+workers); }
	 */
...
class WebServerWorker /* extends WebServer */implements HttpConstants, Runnable {
	...
	void handleClient() throws IOException {
		...
		try {
			 ...
			 * File.separatorChar); if (fname.startsWith(File.separator)) {
			 * fname = fname.substring(1); } File targ = new
			 * File(WebServer.root, fname); if (targ.isDirectory()) { File ind =
			 * new File(targ, "index.html"); if (ind.exists()) { targ = ind; } }
			 * boolean OK = printHeaders(targ, ps); if (doingGet) { if (OK) {
			 * sendFile(targ, ps); } else { send404(targ, ps); } }
			 */
		} finally {
			s.close();
		}
