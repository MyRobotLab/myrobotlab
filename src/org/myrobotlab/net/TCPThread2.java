...
public class TCPThread2 extends Thread {
  ...
  public void run() {
    try {
      ...
      while (socket != null && isRunning) {
         ...
         * sender / name re-write are trivial - the danger & difficulty comes
         * when names are embedded in the data payload - such as register,
        ...
        if (msg.method.equals("register")) {
          ...
          // "register", inboundMsg));
          // Runtime.register(si, uri);// <-- not an INVOKE !!! // - RECENTLY
          // REMOVED !!!!!!
  ...
  public synchronized void send(Message msg) {
    try {
       ...
       * msg.data[0]).getName(); log.info(String.format("serializing %s", n));
       * if (n.equals("r1")) { log.info("here"); // return; }
       * 
       * }
       */
