...
public class Memory {
	...
	public Object put(String parentPath, Node node) {
		 ...
		 * pos0 = 0; int pos1 = parentPath.indexOf("/"); while (pos1 != -1) {
		 * String parentKey = parentPath.substring(pos0, pos1); pos0 = pos1;
		 * pos1 = parentPath.indexOf("/", pos0 + 1); String childKey =
		 * parentPath.substring(pos0 + 1, pos1);
		 * log.info("adding {} -> new node {}", parentKey, childKey);
		 * put(parentKey, new Node(childKey)); if (listener != null) {
	 ...
	 * simplified root case memory.put("", new Node("k1")); memory.put("/k1",
	 * new Node("k2")); memory.put("/k1/k2", new Node("k3")); Node node =
	 * memory.getNode("/k1/k2/k3"); log.info("{}", node.getName());
	 * 
	 * memory.putNode("/k1/k2/k3", "k4"); log.info("{}",
	 * memory.getNode("/k1/k2/k3/k4").getName()); // is this // object of //
	 * node? // memory.get log.info("{}", node.getName());
	 * 
