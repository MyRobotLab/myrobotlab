...
public class Index<T> {
	...
	public static void main(String[] args) throws IOException {
		 ...
		 * reservations.getNode(String.format("global.database.%s"
		 * ,dbConfigKey)); HashMap<String,IndexNode> dbConfig =
		 ...
		 * reservations.get("global.database.config"); dbConfigNode =
		 * reservations
	 ...
	 * log.info("loading config file " + configPath); properties.load(new
	 * FileInputStream(configPath));
	 * 
	 * for (String key : properties.stringPropertyNames()) {
	 * root.buildTree(root, key, properties.getProperty(key));
	 * //IndexNode.buildTree(root, key, properties.getProperty(key)); } } catch
	 * (Exception e) { log.error("config did not load from location " +
	 * configPath); Logging.logException(e); } }
	 */
