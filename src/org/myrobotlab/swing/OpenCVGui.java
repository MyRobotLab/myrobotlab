...
public class OpenCVGui extends ServiceGui implements ListSelectionListener, VideoGUISource, ActionListener {
  ...
  private ActionListener captureListener = new ActionListener() {
    ...
    public void actionPerformed(ActionEvent e) {
       ...
       * OpenCV's actual variables should ONLY be done here otherwise invalid
       * states may occur while a capture is running the model is to set all the
       * data of the gui just before the capture request is sent
       */
      VideoProcessor vp = myOpenCV.videoProcessor;

      String selected = (String) grabberTypeSelect.getSelectedItem();

      if ("IPCamera".equals(selected) || "Pipeline".equals(selected) || "ImageFile".equals(selected) || "SlideShow".equals(selected) || "Sarxos".equals(selected)) {
   ...
   * sent to provide the updated state information. Typically "publishState" is
   * the function which provides the event for onState.
   */
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (o == addFilterButton) {
      ...
      /*
       * if ("depth".equals(mode)) { vp.format = "depth"; } else { vp.format =
       * "image"; } // FIXME - broadcastState ???
       */
    ...
    SwingUtilities.invokeLater(new Runnable() {
      ...
      public void run() {
        ...
        if (opencv != null) {
          ...
          log.error("onState for " + myService.getName() + " was called on " + boundServiceName + " with null reference to state info");
        }
  ...
  public void onOpenCVData(OpenCVData data) {
    ...
    if (data.data != null) { // Needed to avoid null pointer exception when
                             // using RemoteAdapter
      if (cframe != null) {
