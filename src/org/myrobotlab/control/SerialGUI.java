...
public class SerialGUI extends ServiceGUI implements ActionListener, ItemListener {
  ...
  /**
   * the gui is no simplified - a single broadcastState() -> getState(Serial) is
   * used to propegate all data which needs updating. Since that is the case a
   * single invokeLater is used. It is unadvised to have more invokeLater in
  ...
  /**
   * publishRX displays the "interpreted" byte it is interpreted by the Serial's
