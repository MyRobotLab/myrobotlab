...
public class PhotoReelWidget extends ServiceGUI {
  ...
  public JComboBox getServices(JComboBox cb) {
     ...
     * 
     * // String [] namesAndClasses = new String[sortedMap.size()]; int i = 0;
     * while (it.hasNext()) { String serviceName = it.next();
     * cb.addItem(serviceName); // ServiceEntry se = services.get(serviceName);
  ...
  /*
   * MAKE NOTE - BECAUSE THERE WERE 2 - (1 called from SensorMonitorGUI) - I got
   * one bug that was fixed in Serialized - (width/pack performance issue) !
   */
