...
public class RuntimeGUI extends ServiceGUI implements ActionListener {
  ...
  public void actionPerformed(ActionEvent event) {
    ...
    if ("info".equals(cmd)) {
      ...
      /*
       * } else if ("upgrade".equals(cmd)) { // INSTALL OF A SERVICE // send to
       * "my" runtime - may be remote myService.send(myRuntime.getName(),
       * "update", entry.name);
       */
       ...
       * (JCheckBoxMenuItem) ae.getSource(); if (m.isSelected()) { ConnectDialog
       * dlg = new ConnectDialog(new JFrame(), "connect to remote logging",
       * "message", this, "127.0.0.1", "6767"); Logging logging =
       * LoggingFactory.getInstance(); logging.addAppender(Appender.REMOTE,
       * dlg.host.getText(), dlg.port.getText()); } else { Logging logging =
       * LoggingFactory.getInstance(); logging.removeAppender(Appender.REMOTE);
       * }
       */
