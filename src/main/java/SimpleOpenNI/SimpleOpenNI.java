...
public class SimpleOpenNI extends ContextWrapper implements SimpleOpenNIConstants {
	...
	public static String getLibraryPathWin() {
		 ...
		 * url); if (url != null) { // Convert URL to string, taking care of spaces
		 * represented by the "%20" // string. String path =
		 * url.toString().replace("%20", " "); int n0 = path.indexOf('/');
		 * 
		 * int n1 = -1; n1 = path.indexOf("/SimpleOpenNI/library"); // exported apps.
		 * 
		 * // In Windows, path string starts with "jar file/C:/..." // so the substring
		 * up to the first / is removed. n0++;
		 * 
	...
	public static int raySphereIntersection(PVector p, PVector dir, PVector sphereCenter, float sphereRadius,
	...
	public static boolean rayTriangleIntersection(PVector p, PVector dir, PVector vec0, PVector vec1, PVector vec2,
			PVector hit) {
	 ...
	 *                   single thread - RunMode_MultiThreaded = Runs the
	...
	public void calcUserCoordsys(PMatrix3D mat) {
		...
		mat.set(matRet[0], matRet[1], matRet[2], 0, matRet[3], matRet[4], matRet[5], 0, matRet[6], matRet[7], matRet[8],
				0, 0, 0, 0, 1);
	}
	...
	/**
	 * Calculates a point in origninal 3d camera coordinate system to the coordinate
	...
	*/
	...
	public void drawCamFrustum() {
		 ...
		 * 
		 * if(hasUserCoordsys()) { // move the camera to the real nullpoint PMatrix3D
		 * mat = new PMatrix3D(); getUserCoordsys(mat); _parent.g.applyMatrix(mat); }
		 * 
		 * // draw cam case _parent.stroke(200,200,0); _parent.noFill();
		 * _parent.g.beginShape(); _parent.g.vertex(270 * .5f,40 * .5f,0.0f);
		 * _parent.g.vertex(-270 * .5f,40 * .5f,0.0f); _parent.g.vertex(-270 * .5f,-40 *
		 * .5f,0.0f); _parent.g.vertex(270 * .5f,-40 * .5f,0.0f);
		 * _parent.g.endShape(PConstants.CLOSE);
		 * 
		 * _parent.g.beginShape(); _parent.g.vertex(220 * .5f,40 * .5f,-50.0f);
		 * _parent.g.vertex(-220 * .5f,40 * .5f,-50.0f); _parent.g.vertex(-220 * .5f,-40
		 * * .5f,-50.0f); _parent.g.vertex(220 * .5f,-40 * .5f,-50.0f);
		 * _parent.g.endShape(PConstants.CLOSE);
		 * 
		 * _parent.g.beginShape(PConstants.LINES); _parent.g.vertex(270 * .5f,40 *
		 * .5f,0.0f); _parent.g.vertex(220 * .5f,40 * .5f,-50.0f);
		 * 
		 * _parent.g.vertex(-270 * .5f,40 * .5f,0.0f); _parent.g.vertex(-220 * .5f,40 *
		 * .5f,-50.0f);
		 * 
		 * _parent.g.vertex(-270 * .5f,-40 * .5f,0.0f); _parent.g.vertex(-220 * .5f,-40
		 * * .5f,-50.0f);
		 ...
		 * 
		 * float valueH = distDepth * _parent.tan(hFieldOfView() * .5f); float valueV =
		 * distDepth * _parent.tan(vFieldOfView() * .5f);
		 * 
	...
	/**
	 * Enable the irMap data collection ir is only available if there is no rgbImage
	...
	/**
	 * Enable the irMap data collection ir is only available if there is no irImage
	 * activated at the same time
	 * 
	 * @param width  int
	 * @param height int
	...
	public float getJointOrientationSkeleton(int userId, int joint, PMatrix3D jointOrientation) {
		...
		jointOrientation.set(mat[0], mat[1], mat[2], 0, mat[3], mat[4], mat[5], 0, mat[6], mat[7], mat[8], 0, 0, 0, 0,
				1);
	...
	public void getUserCoordsys(PMatrix3D mat) {
		...
		mat.set(matRet[0], matRet[1], matRet[2], matRet[3], matRet[4], matRet[5], matRet[6], matRet[7], matRet[8],
				matRet[9], matRet[10], matRet[11], matRet[12], matRet[13], matRet[14], matRet[15]);
	...
	public void getUserCoordsysTransMat(PMatrix3D xformMat) {
		...
		xformMat.set(mat[0], mat[1], mat[2], mat[3], mat[4], mat[5], mat[6], mat[7], mat[8], mat[9], mat[10], mat[11],
	...
	protected void onCompletedGestureCb(int gestureId, Vec3f pos) {
		try {
			_completedGestureMethod.invoke(_handCbObject,
					new Object[] { this, (int) gestureId, new PVector(pos.x(), pos.y(), pos.z()) });
		} catch (Exception e) {
		}
	}
	...
	protected void onNewHandCb(int handId, Vec3f pos) {
		try {
			_newHandMethod.invoke(_handCbObject,
					new Object[] { this, (int) handId, new PVector(pos.x(), pos.y(), pos.z()) });
		} catch (Exception e) {
		}
	}
	...
	protected void onTrackedHandCb(int handId, Vec3f pos) {
		try {
			_trackedHandMethod.invoke(_handCbObject,
					new Object[] { this, (int) handId, new PVector(pos.x(), pos.y(), pos.z()) });
		} catch (Exception e) {
		}
	}
	...
	private void setupHandCB(Object obj) {
		...
		_trackedHandMethod = getMethodRef(obj, "onTrackedHand",
				new Class[] { SimpleOpenNI.class, int.class, PVector.class });
		...
		_inProgressGestureMethod = getMethodRef(obj, "onProgressGesture",
				new Class[] { SimpleOpenNI.class, int.class });
		_abortedGestureMethod = getMethodRef(obj, "onAbortedGesture", new Class[] { SimpleOpenNI.class, int.class });
		_completedGestureMethod = getMethodRef(obj, "onCompletedGesture",
				new Class[] { SimpleOpenNI.class, int.class, PVector.class });
	}
	...
	protected void updateDepthRealWorld() {
		...
		for (int i = 0; i < _depthMapRealWorld.length; i++) {
			_depthMapRealWorld[i].set(_depthMapRealWorldXn[index++], _depthMapRealWorldXn[index++],
	 ...
	 * catch (Exception e) { } }
	 * 
	 * // hands protected void onCreateHandsCb(long nId, XnPoint3D pPosition, float
	 * fTime) { try { _createHandsMethod.invoke(_handCbObject, new Object[] {
	 * (int)nId,new
	 * PVector(pPosition.getX(),pPosition.getY(),pPosition.getZ()),fTime}); } catch
	 * (Exception e) {} }
	 * 
	 * protected void onUpdateHandsCb(long nId, XnPoint3D pPosition, float fTime) {
	 * try { _updateHandsMethod.invoke(_handCbObject, new Object[] { (int)nId,new
	 * PVector(pPosition.getX(),pPosition.getY(),pPosition.getZ()),fTime}); } catch
	 * (Exception e) {} }
	 * 
	 ...
	 * catch (Exception e) {} }
	 * 
	 * protected void onRecognizeGestureCb(String strGesture, XnPoint3D pIdPosition,
	 * XnPoint3D pEndPosition) { try {
	 ...
	 * float fProgress) { try { _progressGestureMethod.invoke(_gestureCbObject, new
	 * Object[] { strGesture, new
	 * PVector(pPosition.getX(),pPosition.getY(),pPosition.getZ()), fProgress }); }
	 * catch (Exception e) {} }
	 * 
	 * // nite callbacks protected void onStartSessionCb(XnPoint3D ptPosition) { try
	 * { _startSessionMethod.invoke(_sessionCbObject, new Object[] { new
	 ...
	 * (Exception e) {} }
	 * 
	 * protected void onFocusSessionCb(String strFocus, XnPoint3D ptPosition, float
	 * fProgress) { try { _focusSessionMethod.invoke(_sessionCbObject, new Object[]
	 * {strFocus, new
