...
public class TcpThread extends Thread {
	...
	public void run() {
		try {
			...
			while (socket != null && isRunning) {
				...
				// nice for debugging
				if (msgLog != null) {
					msgLog.write(String.format("%s <-- %s - %s\n", myService.getName(), uri, CodecUtils.toJson(msg))
							.getBytes());
				}
				...
				if (uri == null) {
					protocolKey = new URI(
							String.format("tcp://%s:%d", socket.getInetAddress().getHostAddress(), socket.getPort()));
					String mrlURI = String.format("mrl://%s/%s", myService.getName(), protocolKey.toString());
					uri = new URI(mrlURI);
				}

				/**
				 * mrl works similar to - router x-forwarded in that it re-writes names in order
				 * to provide an abstraction to a remote system. This can prevent name collision
				 * and add clarity to remote system names - msg sender / name re-write are
				 * trivial - the danger &amp; difficulty comes when names are embedded in the
				 * data payload - such as register, addListener and other(?) methods - for
				 * example - service names as parameters ! - which "should" only happen with
				 * incorrect user scripts.. because registration is where all info regarding
				 * foreign service names should come from
				 */
				...
				if ("publishState".equals(msg.method)) { // this is to a specific
					...
					if (msgData != null) {
						if (msg.data.length == 0) {
							log.error(
									"*** a publishState was sent without a service - you probably want to send broadcastState ! {} {}**",
									msg.sender, msg.data.length);
							return;
						}
				...
				if (msg.method.equals("register")) {
					...
					if (foreignEnvironment == null) {
						...
						while (it.hasNext()) {
							...
							Message outbound = Message.createMessage(myService, myService.getName(), "sendRemote",
	...
	public synchronized void send(Message msg) {
		try {
			 ...
			 * null){ for (Object o : msg.data){ log.info(String.format("sending %s",
