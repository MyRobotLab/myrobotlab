	...
	public void run() {
		...
		while (isScanning && myService.isRunning()) {
			try {
				 ...
				 * InetAddress.getByName("255.255.255.255"), 6767); dsocket.send(sendPacket);
				 * myService.info( ">>> Request packet sent to: 255.255.255.255 (DEFAULT)"); }
				...
				while (interfaces.hasMoreElements()) {
					...
					for (InterfaceAddress interfaceAddress : ni.getInterfaceAddresses()) {
						...
						try {
							log.info("sending to {} {} {}", ni.getName(), broadcast.getHostAddress(),
									ni.getDisplayName());

							if (interfaceAddress.getNetworkPrefixLength() == -1) {
								log.warn("jdk bug for interface %s network prefix length == -1",
							...
							myService.info(">>> Request packet sent to: " + broadcast.getHostAddress() + "; Interface: "
									+ ni.getDisplayName());
						} catch (Exception e) {
							myService.error(e);
						}
				...
				try {
					...
					while (listening) {
						...
						// We have a response
						myService
								.info(String.format("response from : %s", receivePacket.getAddress().getHostAddress()));

						// Check if the message is correct - JSON ?
						ObjectInputStream inBytes = new ObjectInputStream(
						 ...
						 * (message.equals("DISCOVER_FUIFSERVER_RESPONSE")) { // DO SOMETHING WITH THE
						 * SERVER'S IP (for example, store it in // your controller) // Controller_Base
						 * .setServerIp(receivePacket.getAddress()); log.info( String.format(
