...
public class UdpServer implements Runnable {
	...
	public void run() {
		...
		try {
			...
			log.info("{} UdpServer listening on {}:{}", myService.getName(), serverSocket.getLocalAddress(),
			...
			while (isRunning) {
				...
				try {
					 ...
					 * URI(String.format("tcp:/%s:%d", dgram.getAddress(), dgram.getPort()))); //
					 * send them back for (int i = 0; i < conn.size(); ++i) { Message newConnMsg =
					 * createMessage(null, "publishConnection", conn); byte[] msgBuf =
					 * org.myrobotlab.codec.CodecUtils.getBytes(newConnMsg); DatagramPacket dgp =
					 * new DatagramPacket(msgBuf, msgBuf.length, dgram.getAddress(),
					 * dgram.getPort()); socket.send(dgp); }
					 * 
					 * // we will have to search for them again } else if
					 * ("publishConnection".equals(msg.method)) { myService.invoke("onCommOptions",
					 * msg.data[0]); } else
					 */
					...
					if ("register".equals(msg.method)) {
						...
						URI protocolKey = new URI(String.format("udp://%s:%d",
