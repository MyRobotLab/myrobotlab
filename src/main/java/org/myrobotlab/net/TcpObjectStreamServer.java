...
public class TcpObjectStreamServer implements Runnable {
  public final static Logger log = LoggerFactory.getLogger(TcpObjectStreamServer.class);
   ...
   */
  transient private HashMap<URI, TcpObjectThread> tcpClientList = new HashMap<URI, TcpObjectThread>();
  ...
  public TcpObjectStreamServer(RemoteAdapter s) {
    myService = s;
  }
  ...
  public void run() {
    try {
      ...
      myService.info(String.format("TcpObjectStreamServer listening on %s", serverSocket.getLocalSocketAddress()));
      ...
      while (isRunning) {
        ...
        TcpObjectThread tcp = new TcpObjectThread(myService, uri, clientSocket);
  ...
  public HashMap<URI, Connection> broadcastHeartbeat() {
    for (Map.Entry<URI, Connection> entry : myService.connections.entrySet()) {
      ...
      if (uri.getScheme().equals("tcp")) {
        TcpObjectThread tcp = tcpClientList.get(uri);
        log.info("" + tcp);
  ...
  public void sendTcp(URI uri, Message msg) {
    ...
    try {
      if (tcpClientList.containsKey(uri)) {
        ...
        // port)
        tcp = new TcpObjectThread(myService, uri, null);
        tcpClientList.put(uri, tcp);
        // FIXME - refactor out
        myService.connections.put(uri, tcp.data);
        myService.broadcastState();
