...
public class PImage implements PConstants, Cloneable {
	...
	/**
	 * Format for this image, one of RGB, ARGB or ALPHA. note that RGB images still
	 * require 0xff in the high byte because of how they'll be manipulated by other
	 * functions
	 */
	public int format;
	 ...
	 * have to set the modified region to include the min/max // of the coordinates
	 * coming in. // also, mustn't get the pixels for the section that's // already
	 * been marked as modified. gah. // too complicated, just throw an error String
	 * msg = "getPixels(x, y, w, h) cannot be used multiple times. " +
	 * "Use getPixels() once to get the entire image instead."; throw new
	 * RuntimeException(msg); }
	 * 
	 * if (imageMode == CORNER) { // x2, y2 are w/h x2 += x1; y2 += y1; }
	 * 
	 * if (pixels == null) { // this is a java 1.3 buffered image if (image == null)
	 * { // this is just an error throw new RuntimeException(
	 ...
	 * Integer.TYPE, Integer.TYPE }); getRgbMethod.invoke(image, new Object[] { new
	 * Integer(x1), new Integer(y1), new Integer(x2 - x1 + 1), new Integer(y2 - y1 +
	 * 1), pixels, new Integer(0), new Integer(width) });
	 * 
	 * } catch (Exception e) { e.printStackTrace(); } } return pixels; // just to be
	 * nice }
	 */
	...
	private static int blend_darkest(int a, int b) {
		...
		return (low(((a & ALPHA_MASK) >>> 24) + f, 0xff) << 24
				...
				| mix(a & GREEN_MASK, low(a & GREEN_MASK, ((b & GREEN_MASK) >> 8) * f), f) & GREEN_MASK
				| mix(a & BLUE_MASK, low(a & BLUE_MASK, ((b & BLUE_MASK) * f) >> 8), f));
	}
	...
	private static int blend_sub_pin(int a, int b) {
		...
		return (low(((a & ALPHA_MASK) >>> 24) + f, 0xff) << 24
				...
				| high(((a & GREEN_MASK) - ((b & GREEN_MASK) >> 8) * f), BLUE_MASK) & GREEN_MASK
	 ...
	 * width; int th = height;
	 * 
	 * if (tx < 0) { // say if target x were -3 sx -= tx; // source x -(-3) (or add
	 * 3) sw += tx; // source width -3 tw += tx; // target width -3 tx = 0; //
	 * target x is zero (upper corner) } if (ty < 0) { sy -= ty; sh += ty; th += ty;
	 * ty = 0; } if (tx + tw > width) { int extra = (tx + tw) - width; sw -= extra;
	 * tw -= extra; } if (ty + th > height) { int extra = (ty + th) - height; sh -=
	 * extra; sw -= extra; }
	 * 
	...
	/*
	 * Create a new RGB (alpha ignored) image of a specific size. All pixels are set
	 * to zero, meaning black, but since the alpha is zero, it will be transparent.
	 */
	...
	public void blend(int sx, int sy, int dx, int dy, int mode) {
		if ((dx >= 0) && (dx < width) && (sx >= 0) && (sx < width) && (dy >= 0) && (dy < height) && (sy >= 0)
				&& (sy < height)) {
	...
	public void blend(PImage src, int sx, int sy, int dx, int dy, int mode) {
		if ((dx >= 0) && (dx < width) && (sx >= 0) && (sx < src.width) && (dy >= 0) && (dy < height) && (sy >= 0)
				&& (sy < src.height)) {
	...
	public void blend(PImage src, int sx1, int sy1, int sx2, int sy2, int dx1, int dy1, int dx2, int dy2, int mode) {
		...
		if ((src == this) && intersect(sx1, sy1, sx2, sy2, dx1, dy1, dx2, dy2)) {
			blit_resize(get(sx1, sy1, sx2 - sx1, sy2 - sy1), 0, 0, sx2 - sx1 - 1, sy2 - sy1 - 1, pixels, width, height,
					dx1, dy1, dx2, dy2, mode);
		} else {
	...
	/**
	 * Duplicate an image, returns new PImage object. The pixels[] array for the new
	 * object will be unique and recopied from the source image.
	 */
	@Override
	 ...
	 * image to opaque <LI>filter(THRESHOLD) converts the image to black and white.
	 * </UL>
	 */
	 ...
	 * posterize the image to between 2 and 255 levels. <LI>filter(THRESHOLD, float
	 * center) allows you to set the center point for the threshold. It takes a
	 * value from 0 to 1.0. </UL>
	 */
	...
	private int filter_bilinear() {
		...
		r = ((ul * ((cUL & RED_MASK) >> 16) + ll * ((cLL & RED_MASK) >> 16) + ur * ((cUR & RED_MASK) >> 16)
				+ lr * ((cLR & RED_MASK) >> 16)) << PREC_RED_SHIFT) & RED_MASK;

		g = ((ul * (cUL & GREEN_MASK) + ll * (cLL & GREEN_MASK) + ur * (cUR & GREEN_MASK)
				+ lr * (cLR & GREEN_MASK)) >>> PRECISIONB) & GREEN_MASK;

		b = (ul * (cUL & BLUE_MASK) + ll * (cLL & BLUE_MASK) + ur * (cUR & BLUE_MASK)
				+ lr * (cLR & BLUE_MASK)) >>> PRECISIONB;

		a = ((ul * ((cUL & ALPHA_MASK) >>> 24) + ll * ((cLL & ALPHA_MASK) >>> 24) + ur * ((cUR & ALPHA_MASK) >>> 24)
	 ...
	 * the value will get its high bits set, just to avoid cases where they haven't
	 * been set already. <P> If the image is in ALPHA format, this returns a white
	 * color that has its alpha value set. <P> This function is included primarily
	 * for beginners. It is quite slow because it has to check to see if the x, y
	 * that was provided is inside the bounds, and then has to check to see what
	 * image type it is. If you want things to be more efficient, access the
	 ...
	 * fully opaque. Gray values will be in-between steps. <P> Strictly speaking the
	 * "blue" value from the source image is used as the alpha color. For a fully
	 * grayscale image, this is correct, but for a color image it's not 100%
