...
public class PointsShape extends Shape3D implements GeometryUpdater {
	...
	/**
	 * the gap between depth positions being sampled this is an optimization of only
	 * sampling modulus SAMPLE_FREQ on the X axis data
	 */
	...
	/**
	 * used to make updateDepthCoords() wait until GeometryUpdater.updateData() has
	 * finished an update
	 */
	 ...
	 * and the resolution per fixed area decreases as an inverse to the distance
	 * from the sensor
	 * 
	 ...
	 * sample every SAMPLE_FREQ point to reduce the arrays size. Each point requires
	 * 3 floats in the coords array (x, y, z) and 3 floats in the colours array (R,
	 * G, B) + Alpha
	 * 
	...
	private void createGeometry() {
		 ...
		 * int ptIdx = (dpIdx / SAMPLE_FREQ) * 3; // calc point index // if (ptIdx <
		 * MAX_POINTS * 3) { // is there enough space? coords[index] = index%IM_WIDTH *
		 * 0.01f;// * xScale; // x coord coords[index + 1] = (index/3)/IM_WIDTH *
		 * 0.01f;// * yScale; // y coord coords[index + 2] = 1f; // z coord (will change
		 * later)
		 * 
		 * // initial point colour is white (will change later) colors[index] = 1.0f;
		 * colors[index + 1] = 1.0f; colors[index + 2] = 1.0f; // colors[index + 3] =
		 * 1.0f;
		 * 
	...
	private void printCoord(float[] coords, int xIdx) {
		System.out.println("" + xIdx + ". depth coord (x,y,z): (" + coords[xIdx] + ", " + coords[xIdx + 1] + ", "
				+ coords[xIdx + 2] + ")");
	}
	 ...
	 * ViewerPanel example: red in forground (and for no depth), changing to violet
	 * in the background
	 */
	...
	public void updateData(Geometry geo) {
		...
		for (int depthDataIndex = 0; depthDataIndex < data.length; ++depthDataIndex) {
			...
			{
				log.warn("midpoint ijr ({},{},{}) => xyz ({},{},{})  ({},{},{}) ", i, j, r, x, y, z, 39.3701 * x,
						39.3701 * y, 39.3701 * z);
			}
			/*
		 ...
		 * SAMPLE_FREQ == 0) { // save this z-coord int zCoordIdx = (i / SAMPLE_FREQ) *
		 * 3 + 2; if (zCoordIdx < coords.length) { coords[zCoordIdx] = -zCoord; //
		 * negate so depths are spread out along -z axis, away from // camera //
		 * printCoord(coords, zCoordIdx-2); updateColour(zCoordIdx - 2, zCoord); } } }
		 */
	...
	/*
	 * Use new depth buffer data to update the PointsArray inside the Java 3D scene.
	 * This method is repeatedly called by DepthReader as the depth buffer changes.
