...
public interface PConstants {
	...
	static public final int DR = 3; // TODO needs to not be shared, this is a
	// material property
	...
	/**
	 * Same as Float.MAX_VALUE, but included for parity with MIN_VALUE, and to avoid
	 * teaching static methods on the first day.
	 */
	static final float MAX_FLOAT = Float.MAX_VALUE;
	/**
	 * Note that Float.MIN_VALUE is the smallest <EM>positive</EM> value for a
	 * floating point number, not actually the minimum (negative) value for a float.
	 * This constant equals 0xFF7FFFFF, the smallest (farthest negative) value a
	 * float can have before it hits NaN.
	 */
	 ...
	 * transformations like any other shapes. Changed value in 0093 to not interfere
	 * with LEFT, CENTER, and RIGHT.
	 */
	 ...
	 * textMode(SHAPE) will be ignored and textMode(MODEL) will be used instead. For
	 * this reason, be sure to call textMode() <EM>after</EM> calling textFont().
	 * 
	 * Currently, textMode(SHAPE) is only supported by OPENGL mode. It also requires
	 * Java 1.2 or higher (OPENGL requires 1.4 anyway)
	 */
	...
	/**
	 * This constant identifies the texture target GL_TEXTURE_2D, that is, textures
	 ...
	 * KeyEvent.KEYCODE_DPAD_UP; static final int DOWN = KeyEvent.KEYCODE_DPAD_DOWN;
	 * static final int LEFT = KeyEvent.KEYCODE_DPAD_LEFT; static final int RIGHT =
	 * KeyEvent.KEYCODE_DPAD_RIGHT;
	 * 
	 * // These seem essential for most sketches, so they're included. // Others can
	 * be found in the KeyEvent reference: //
	 ...
	 * KeyEvent.KEYCODE_MENU; static final int DPAD = KeyEvent.KEYCODE_DPAD_CENTER;
	 */
