...
public class ApiService extends Api {
	...
	public Object process(MessageSender sender, OutputStream out, Message msgFromUri, String data) throws Exception {
		...
		if (msgFromUri.data != null) {
			...
			paramTypes = MethodCache.getCandidateOnOrdinalSignature(si.getClass(), msgFromUri.method,
					encodedArray.length);
	...
	public static ApiDescription getDescription() {
		ApiDescription desc = new ApiDescription("message", "{scheme}://{host}:{port}/api/service",
				"http://localhost:8888/api/service/runtime/getUptime",
