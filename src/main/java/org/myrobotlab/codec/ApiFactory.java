...
public class ApiFactory {
	...
	public static List<ApiDescription> getApis() {
		...
		for (String key : apiKeys) {
			try {
				Class<?> clazz = Class.forName(String.format("org.myrobotlab.codec.Api%s",
	 ...
	 * parts[2]
	 * 
	 * @param sender     - MessageSender - if apiKey specifies a message based
	 *                   processor, the router will convert and send
	 * @param out        - an OutputStream from the requestor, results (if any) will
	 *                   be sent back to this OutputStream
	...
	public Object process(MessageSender sender, OutputStream out, String requestUri, String data) throws Exception {
		...
		if (!processors.containsKey(apiKey)) {
			...
			Object p = null;
			try {
				className = String.format("org.myrobotlab.codec.Api%s",
						apiKey.substring(0, 1).toUpperCase() + apiKey.substring(1));
	 ...
	 * parts.length < 4){ return "runtime"; // relies on singlton standard ! } else
	 * { return parts[3]; } }
	 * 
