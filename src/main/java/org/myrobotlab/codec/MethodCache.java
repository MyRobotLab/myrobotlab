...
public class MethodCache {
	...
	static final public Class<?>[] getCandidateOnOrdinalSignature(Class<?> clazz, String methodName, int ordinal)
		...
		if (cache.containsKey(signature)) {
				...
				if (methodName.equals(method.getName())) {
					...
					if (ordinal == pTypes.length) {
						...
						for (int i = 0; i < pTypes.length; ++i) {
							...
							/*
							 * BAD ASSUMPTION - SOME CODECs have a default class they serialize from for
							 * List Map HashSet etc..
							 */
							/*
			...
			if (methodScore.size() > 0) {
				...
				throw new NoSuchMethodException(String.format("could not find %s.%s(ordinal %d) in declared methods",
						clazz.getSimpleName(), methodName, ordinal));
			}
		}
