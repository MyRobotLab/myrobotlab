...
/**
 * handles all encoding and decoding of MRL messages or api(s) assumed context - services can add an assumed context as a prefix
...
public class CodecUtils {
  ...
  /**
   * a method signature map based on name and number of methods - the String[] will be the keys into the methodCache A method key is
   * generated by input from some encoded protocol - the method key is object name + method name + parameter number - this returns a
   * full method signature key which is used to look up the method in the methodCache
   */
  ...
  /**
   * most lossy protocols need conversion of parameters into correctly typed elements this method is used to query a candidate
   * method to see if a simple conversion is possible
   * 
   * @param clazz
   *          the class
   * @return true/false
