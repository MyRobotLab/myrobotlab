...
public class CodecUtils {
	...
	private transient static Gson gson = new GsonBuilder().setDateFormat("yyyy-MM-dd HH:mm:ss.SSS").setPrettyPrinting()
	...
	public static final Set<String> WRAPPER_TYPES_CANONICAL = new HashSet<String>(Arrays.asList(
			Boolean.class.getCanonicalName(), Character.class.getCanonicalName(), Byte.class.getCanonicalName(),
			Short.class.getCanonicalName(), Integer.class.getCanonicalName(), Long.class.getCanonicalName(),
			Float.class.getCanonicalName(), Double.class.getCanonicalName(), Void.class.getCanonicalName()));
	 ...
	 * will be the keys into the methodCache A method key is generated by input from
	 * some encoded protocol - the method key is object name + method name +
	 * parameter number - this returns a full method signature key which is used to
	 * look up the method in the methodCache
	 */
	...
	static final public String getMsgKey(Message msg) {
		return String.format("msg %s.%s --> %s.%s(%s) - %d", msg.sender, msg.sendingMethod, msg.name, msg.method,
				CodecUtils.getParameterSignature(msg.data), msg.msgId);
	}

	static final public String getMsgTypeKey(Message msg) {
		return String.format("msg %s.%s --> %s.%s(%s)", msg.sender, msg.sendingMethod, msg.name, msg.method,
	...
	public static final String msgToBase64(Message msg) {
		...
		try {
			...
			objectStream.close();
			dataStream.close();
			String base64 = String.format("%s://%s", SCHEME_BASE64,
	...
	public static boolean setJSONPrettyPrinting(boolean b) {
		if (b) {
			gson = new GsonBuilder().setDateFormat("yyyy-MM-dd HH:mm:ss.SSS").setPrettyPrinting().disableHtmlEscaping()
					.create();
		} else {
