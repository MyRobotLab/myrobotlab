...
final public class Search {
	 ...
	 * other words, negamax. It basically only searches capture sequences and allows
	 * the evaluation function to cut the search off (and set alpha). The idea is to
	 * find a position where there isn't a lot going on so the static evaluation
	 * function will work.
	 */
	...
	int search(int alpha, int beta, int depth) throws StopSearchingException {
		/*
		 * we're as deep as we want to be; call quiesce() to get a reasonable score and
		 * return it.
		 */
		if (depth == 0)
		 ...
		 * can't simply return 0) then check to see if the position is a repeat. if so,
		 * we can assume that this line is a draw and return 0.
		 */
		...
		while (i.hasNext()) {
			...
			if (betterMove) {
				/*
				 * this move caused a cutoff, so increase the history value so it gets ordered
				 * high next time we can search it
				 */
