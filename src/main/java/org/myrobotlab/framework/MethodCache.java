...
public class MethodCache {
	...
	/**
	 * Cache for Methods In fact this is a map (with classes as keys) of a map (with
	...
	public Method getMethod(Class clazz, String methodName, Class[] parameterTypes) throws NoSuchMethodException {
		...
		try {
			...
			if (!clazz.isPrimitive() && !className.startsWith("java.") && !className.startsWith("javax.")) {
				...
				/*
				 * try { // Class helper = ClassUtils.forName(className + "_Helper"); // method
				 * = helper.getMethod(methodName, parameterTypes); } catch
