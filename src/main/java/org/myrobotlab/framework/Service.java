...
public abstract class Service extends MessageService
	...
	/**
	 * contains all the meta data about the service - pulled from the static method
	 * getMetaData() each instance will call the method and populate the data for an
	 * instance
	 * 
	 */
	...
	/**
	 * a radix-tree of data -"DNA" Description of Neighboring Automata ;) this is a
	 * 'master build plan' for the service
	 * 
	 ...
	 * instance is in Chicago and you just say "Bob" I will think your talking about
	 * "Chicago Bob" :)
	 */
	...
	public final String DATA_INSTANCE_DIR = "data" + File.separator + OpenCV.class.getSimpleName() + File.separator
			+ name;
	public final String RESOURCE_INSTANCE_DIR = "resource" + File.separator + OpenCV.class.getSimpleName()
			+ File.separator + name;
	...
	/**
	 * overload this if your service needs other environmental or dependencies to be
	 * ready
	 */
	...
	public static TreeMap<String, ServiceReservation> mergeDna(TreeMap<String, ServiceReservation> dna,
			TreeMap<String, ServiceReservation> rna) {
	 ...
	 * @param myKey        - key (name) instance of the class currently under
	 *                     construction
	 * @param serviceClass - type of class being constructed
	 * @param comment      - added comment
	 * @return a map
	 */
	static public TreeMap<String, ServiceReservation> buildDna(TreeMap<String, ServiceReservation> dna, String myKey,
			String serviceClass, String comment) {
		...
		try {
			...
			for (ServiceReservation templatePeer : peers.values()) {
				...
				if (rna == null) {
					...
					log.info("dna adding new key {} {} {} {}", fullKey, templatePeer.actualName,
							templatePeer.fullTypeName, comment);
					...
					buildDna(dna, Peers.getPeerKey(myKey, templatePeer.key), templatePeer.fullTypeName,
							templatePeer.comment);
				}
	 ...
	 * 
	 * @param key unique identification of the peer service used by the composite
			...
			for (Entry<String, ServiceReservation> entry : peers.entrySet()) {
				...
				if (!dnaPool.containsKey(fullKey)) {
					...
					if (template.key.equals(template.actualName) && !template.isRoot) {
						...
						sr = new ServiceReservation(fullKey, actualName, template.fullTypeName, template.comment,
								template.isRoot);
					...
					if (sr.fullTypeName == null) {
						log.info("no type name in reservation, replacing with standard type - {}",
	...
	public void movePeerDna(String myKey, String actualName, String fullTypeName, String comment) {
		...
		if (meta != null) {
			...
			for (Entry<String, ServiceReservation> reservation : peers.entrySet()) {
				...
				ServiceReservation sr = new ServiceReservation(movedActual, movedActual, templateSr.fullTypeName,
						templateSr.comment);
	...
	/**
	 * This method re-binds the key to another name. An example of where this would
	 * be used is within Tracking there is an Servo service named "x", however it
	 * may be desired to bind this to an already existing service named "pan" in a
	 * pan/tilt system
	 * 
	 * @param key     key internal name
	 * @param newName new name of bound peer service
	 ...
	 * 
	 * @param topicMethod    - method when called, it's return will be sent to the
	...
	public void addListener(String topicMethod, String callbackName, String callbackMethod) {
		...
		if (outbox.notifyList.containsKey(listener.topicMethod.toString())) {
			...
			if (!found) {
				log.debug("adding addListener from {}.{} to {}.{}", this.getName(), listener.topicMethod,
	 ...
	 * the return value would change depending on if the service is local or not.
	 * 
	...
	/**
	 * called typically from a remote system When 2 MRL instances are connected they
	 * contain serialized non running Service in a registry, which is maintained by
	 * the Runtime. The data can be stale.
	 * 
	 ...
	 * Listening service (STT) such that when the system is speaking it does not try
	 * to listen &amp; act on its own speech (feedback loop)
	 * 
	 * FIXME - the SwingGui currently has attachGUI() and detachGUI() - these are to
	 * bind Services with their swing views/tab panels. It should be generalized to
	 * this attach method
	 * 
	 * @param subpath s
	 * 
	...
	final public Object invoke(Message msg) {
		...
		if (log.isDebugEnabled()) {
			log.debug("--invoking {}.{}({}) {} --", name, msg.method, CodecUtils.getParameterSignature(msg.data),
					msg.msgId);
		}
	...
	final public Object invokeOn(Object obj, String method, Object... params) {
		...
		try {
			...
			log.debug("no such method {}.{} - attempting upcasting", c.getSimpleName(),
	...
	public boolean load(Object o, String inCfgFileName) {
		...
		if (inCfgFileName == null) {
			filename = String.format("%s%s%s.json", cfgDir, File.separator,
					String.format("%s-%s", getClass().getSimpleName(), getName()));
		} else {
	...
	/**
	 * Creating a message function call - without specifying the recipients - static
	 * routes will be applied this is good for Motor drivers - you can swap motor
	 * drivers by creating a different static route The motor is not "Aware" of the
	 * driver - only that it wants to method="write" data to the driver
	 */
	 ...
	 * composite
	 * 
	 * @param key            internal key name of peer service
	 * @param simpleTypeName type of service
	 * @param comment        comment detailing the use of the peer service within
	 *                       the composite
	 */
	...
	public boolean save() {
		...
		try {
			File cfg = new File(String.format("%s%s%s.json", cfgDir, File.separator,
					String.format("%s-%s", getClass().getSimpleName(), getName())));
	...
	public void unsubscribe(String topicName, String topicMethod, String callbackName, String callbackMethod) {
		...
		cm.send(Message.createMessage(this, topicName, "removeListener",
	...
	/**
	 * error only channel publishing point versus publishStatus which handles info,
	 * warn &amp; error
	 * 
	 * @param status status
	 * @return the status
	 ...
	 * simply return.
	 * 
	 * If its detached to this service, it should first detach itself, modifying its
	 * own data if necessary. The last thing it should do is call the parameterized
	 * service's detach. This gives the other service an opportunity to detach. e.g.
	 * 
	 * <pre>
	 * 
	 ...
	 * </pre>
	 * 
	 * @param service - the service to detach from this service
	 */
	@Override
	public void detach(Attachable service) {
	}
	...
	/**
	 * the "routing" isAttached - when overridden by a service this "routes" to the
	 * appropriate typed isAttached
	 */
	@Override
	 ...
	 * simply return.
	 * 
	 * If its attached to this service, it should first attach itself, modifying its
	 * own data if necessary. The last thing it should do is call the parameterized
	 * service's attach. This gives the other service an opportunity to attach. e.g.
	 * 
	 * <pre>
	 * 
	 ...
	 * </pre>
	 * 
	 * @param service - the service to attach from this service
	 */
	@Override
	public void attach(Attachable service) throws Exception {
		log.info("Service.attach does not know how to attach {} to a {}", service.getClass().getSimpleName(),
