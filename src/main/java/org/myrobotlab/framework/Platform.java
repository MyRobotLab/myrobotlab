...
public class Platform implements Serializable {
	...
	/**
	 * The one big convoluted function to get all the crazy platform specific data.
	 * Potentially, it's done once and only once for a running instance. Most of the
	 * data should be immutable, although the "id"
	 * 
	...
	public static Platform getLocalInstance() {
		...
		if (localInstance == null) {
			 ...
			 * platform.mrlVersion = sb.toString(); } } catch (Exception e) { // no logging
			 * silently die }
			 */
	...
	static public Map<String, String> getManifest() {
		...
		try {
			...
			if (!classPath.startsWith("jar")) {
				System.out.println(String.format("manifest is \"not\" in jar - using file %s/META-INF/MANIFEST.MF",
						f.getAbsolutePath()));
				// File file = new
