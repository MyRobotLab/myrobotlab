...
public class TypeConverter {
	...
	static public Object[] getTypedParamsFromJson(Class<?> clazz, String method, String[] stringParams)
		...
		for (int i = 0; i < methods.length; ++i) {
			...
			if (method.equals(m.getName()) && stringParams.length == types.length) {
				log.debug("method with same ordinal of params found {}.{} - building new converter", method,
						stringParams.length);
				...
				try {
					...
					for (int j = 0; j < types.length; ++j) {
						...
						log.debug("attempting conversion into {} from inbound data {}", pType.getSimpleName(),
								stringParams[j]);
	 ...
	 * 0; i < stringParams.length; ++i) { // static calls on conversion - probably
	 * not thread safe newTypedParams[0] = converter[i].invoke(null,
	 * stringParams[i]); }
	 * 
