...
public class Inbox implements Serializable {
	...
	public void add(Message msg) {
		if ((msg.historyList.contains(name))) {
			log.error("* {} dumping duplicate message {}.{} msgid - {} {}", name, msg.name, msg.method, msg.msgId,
					msg.historyList);
			return;
		}
	...
	/**
	 * Blocks and waits on a message put on the queue of the InBox. Service default
	 * behavior will wait on getMsg for a message, when they recieve a message they
	 * invoke it.
	 * 
	...
	public Message getMsg() throws InterruptedException {
		/*
		 * TODO - remove below - Inbox will call switchboards serializer/deserializer
		 * &amp; communicator send/recieve interface switchboard has references to
		 * serializer and communicator - also all configuration needed At this level ALL
		 * details on where the Message / Message came from should be hidden and
		 * interfaces should be exposed only-
		 */
