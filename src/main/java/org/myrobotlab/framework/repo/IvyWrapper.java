	...
	public void install(String location, String[] serviceTypes) {
		...
		try {
			...
			String[] cmd = new String[] { "-settings", location + "/ivysettings.xml", "-ivy", location + "/ivy.xml",
					"-retrieve", location + "/[originalname].[ext]" };
	...
	public void createIvy(String location, String[] serviceTypes) throws IOException {
		...
		for (String serviceType : serviceTypes) {
			...
			for (ServiceDependency dependency : dependencies) {
				...
				sb.append(String.format(" org=\"%s\" name=\"%s\" rev=\"%s\"", dependency.getOrgId(),
				...
				// <dependency /> or <dependency></dependency> - ie do we have more stuff ?
				...
				if (dependency.getExt() != null) {
					...
					// " <artifact name=\"foo-src\" type=\"%s\" ext=\"%s\" conf=\"provided->master\"
					// />\n",
