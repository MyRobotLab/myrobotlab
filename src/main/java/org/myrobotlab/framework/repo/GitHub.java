...
public class GitHub {
	...
	public static String getPyRobotLabScript(String branch, String serviceType) {
		byte[] script = Http.get(String.format(
				"https://raw.githubusercontent.com/MyRobotLab/pyrobotlab/%s/service/%s.py", branch, serviceType));
		if (script != null) {
	...
	public static Set<String> getServiceScriptNames(String branch) throws Exception {
		...
		byte[] data = Http.get(
				String.format("https://api.github.com/repos/MyRobotLab/pyrobotlab/contents/service?ref=%s", branch));
		if (data != null) {
