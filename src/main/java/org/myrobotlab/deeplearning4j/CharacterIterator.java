...
/**
 * A simple DataSetIterator for use in the GravesLSTMCharModellingExample. Given a text file and a few options, generate feature
 * vectors and labels for training, where we want to predict the next character in the sequence.<br>
 * This is done by randomly choosing a position in the text file, at offsets of 0, exampleLength, 2*exampleLength, etc to start each
 * sequence. Then we convert each character to an index, i.e., a one-hot vector. Then the character 'a' becomes [1,0,0,0,...], 'b'
 * becomes [0,1,0,0,...], etc
 *
...
public class CharacterIterator implements DataSetIterator {
   ...
   */
  public CharacterIterator(String textFilePath, Charset textFileEncoding, int miniBatchSize, int exampleLength, char[] validCharacters, Random rng) throws IOException {
    ...
    if (exampleLength >= fileCharacters.length)
      throw new IllegalArgumentException("exampleLength=" + exampleLength + " cannot exceed number of valid characters in file (" + fileCharacters.length + ")");
  ...
  public DataSet next(int num) {
    ...
    // Why 'f' order here? See http://deeplearning4j.org/usingrnns.html#data section "Alternative: Implementing a custom
    // DataSetIterator"
