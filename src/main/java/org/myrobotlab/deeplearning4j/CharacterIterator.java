...
public class CharacterIterator implements DataSetIterator {
  ...
  public CharacterIterator(String textFilePath, Charset textFileEncoding, int miniBatchSize, int exampleLength, char[] validCharacters, Random rng) throws IOException {
    ...
    System.out.println("Loaded and converted file: " + fileCharacters.length + " valid characters of " + maxSize + " total characters (" + nRemoved + " removed)");
  ...
  public DataSet next(int num) {
    ...
    for (int i = 0; i < currMinibatchSize; i++) {
      ...
      for (int j = startIdx + 1; j < endIdx; j++, c++) {
        int nextCharIdx = charToIdxMap.get(fileCharacters[j]); // Next character
                                                               // to predict
  ...
  private void initializeOffsets() {
    ...
    int nMinibatchesPerEpoch = (fileCharacters.length - 1) / exampleLength - 2; // -2:
                                                                                // for
                                                                                // end
                                                                                // index,
                                                                                // and
