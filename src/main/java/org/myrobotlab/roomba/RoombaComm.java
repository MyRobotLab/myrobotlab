...
public abstract class RoombaComm {
	...
	/**
	 * Compute possible safety fault. Called on every successful updateSensors(). In
	 * normal use, call updateSensors() then check safetyFault().
	 * 
	...
	/**
	 * Connect to a port (for serial, portid is serial port name, for net, portid is
	 * url?)
	 * 
	 * @param portid port
	 ...
	 * @param song    array of songnotes, even entries are notenums, odd are
	 *                duration of 1/6ths
	 */
	...
	/**
	 * Go straight at the current speed for a specified distance. Positive distance
	 * moves forward, negative distance moves backward. This method blocks until the
	 * action is finished.
	 * 
	 ...
	 * 
	 * @return a list available portids, if applicable or empty set if no ports, or
	 * return null if list is not enumerable
	 */
	...
	/**
	 * Play a musical note Does it via the hacky method of defining a one-note song
	 * &amp; playing it Uses up song slot 15. If another note is played before one
	 * is finished, the new note cuts off the old one.
	 * 
	 ...
	 * 
	 * @param sensorList A byte array containing the sensor groups requested to be
	 *                   read
	 * @param returnLen  The number of bytes of data expected to be returned from
	 *                   roomba
	 */
	...
	/**
	 * Reset Roomba after a fault. This takes it out of whatever mode it was in and
	 * puts it into safe mode. This command also syncs the object's sensor state
	 * with the Roomba's by calling updateSensors()
	 * 
	 * @see #startup()
	 * @see #updateSensors()
	 */
	 ...
	 * 
	 * @param packetcode one of SENSORS_ALL, SENSORS_PHYSICAL, SENSORS_INTERNAL, or
	 ...
	 * 
	 * @param angle angle in degrees, positive to spin left, negative to spin right
	 */
	...
	/**
	 * Put Roomba in safe mode. As opposed to full mode. Safe mode is the preferred
	 * working state when playing with the Roomba as it provides some measure of
	 * autonomous self-preservation if it encounters a cliff or is picked up If that
	 * happens it goes into passive mode and must be 'reset()'.
	 * 
	 * @see #reset()
	 */
	public void startup() {
	...
	/*
	 * no button = -1 power = -118 8a pause = -119 89 clean = -120 88 max = -123 85
	 * spot = -124 84 spinleft = -125 81 (8d keyup?) forward = -126 82 (8c?)
	 * spinright = -127 83
	 */
	 ...
	 * twiddling its DD line, often hooked up to the RS-232 DTR line, which may not
	 * be available in some implementations
	 */
	public void wakeup() {
