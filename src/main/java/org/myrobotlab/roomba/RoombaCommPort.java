...
public class RoombaCommPort extends RoombaComm implements SerialDataListener {
	...
	/**
	 * contains a list of all the ports keys are port names (e.g. "/dev/usbserial1")
	 ...
	 * errors deep inside RXTX if an opened port is used before the virtual COM port
	 * is ready. One way to check that it is ready is to look for the DSR line going
	 * high. However, most simple, real serial ports do not do hardware handshaking
	 * so never set DSR high. Thus, if using Bluetooth serial on Windows, do:
	...
	/*
	 * Let you check to see if a port is in use by another Rooomba before trying to
	 * use it.
	 */
