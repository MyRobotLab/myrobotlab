		...
		try {
			...
			while (true) {
				...
				if (numAvailable < maxFrameSize * MAX_BUFFER_RATIO || numSkipped >= maxSkippedFrames) {
					if (numSkipped == 1) {
						log.info("Skipped {} frame in {} ms. Frame Size {}", numSkipped,
								System.currentTimeMillis() - start, mf.getBytes().length);
					} else if (numSkipped > 1) {
						log.info("Skipped {} frames in {} ms. Frame Size {}", numSkipped,
								System.currentTimeMillis() - start, mf.getBytes().length);
					}
					break;
				}
