...
public class OpenCVFilterGoodFeaturesToTrack extends OpenCVFilter {
	...
	public BufferedImage processDisplay(Graphics2D graphics, BufferedImage image) {
		...
		for (int i = 0; i < count[0]; ++i) {
			/*
			 * since there is no subpixel selection - we don't need to round - we can cast x
			 * = Math.round(corners.x()); y = Math.round(corners.y());
			 */
			corners.position(i);
			x = (int) corners.x();
			y = (int) corners.y();
	...
	public IplImage process(IplImage image) {
		...
		cvGoodFeaturesToTrack(grey, eig, temp, corners, countPointer, qualityLevel, minDistance, mask, blockSize,
				useHarris, k);
