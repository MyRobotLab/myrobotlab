	...
	private ArrayList<DetectedFace> extractDetectedFaces(Mat bwImgMat) {
		...
		for (int i = 0; i < faces.size(); i++) {
			...
			if ((mouths.size() == 1) && (eyes.size() == 2) && !rectOverlap(eyes.get(0), eyes.get(1))) {
				...
				mouthRect = new Rect(mouths.get(0).x(), mouths.get(0).y() + face.height() / 3 * 2,
						mouths.get(0).width(), mouths.get(0).height());
	...
	private void drawFaceRects(IplImage image, DetectedFace dFace) {
		...
		if (dFace.getLeftEye() != null) {
			...
			Rect offset = new Rect(dFace.getFace().x() + dFace.getLeftEye().x(),
					dFace.getFace().y() + dFace.getLeftEye().y(), dFace.getLeftEye().width(),
		...
		if (dFace.getRightEye() != null) {
			Rect offset = new Rect(dFace.getFace().x() + dFace.getRightEye().x(),
					dFace.getFace().y() + dFace.getRightEye().y(), dFace.getRightEye().width(),
					dFace.getRightEye().height());
			drawRect(image, offset, CvScalar.BLUE);
		}
		if (dFace.getMouth() != null) {
			Rect offset = new Rect(dFace.getFace().x() + dFace.getMouth().x(),
					dFace.getFace().y() + dFace.getMouth().y(), dFace.getMouth().width(), dFace.getMouth().height());
