...
public class OpenCVFilterAddMask extends OpenCVFilter {
  ...
  public IplImage process(IplImage image) throws InterruptedException {
    if (mask != null) {
      ...
      /*
       * 
       * cvSetImageROI(output, cvRect(0, 0, mask.width(), mask.height())); cvAdd(mask, mask, output); cvResetImageROI(overlay);
       */
      ...
      log.info("here");
      /*
       * // Convert Mat to float data type mask.convertTo(mask, CV_32FC3); background.convertTo(background, CV_32FC3);
       * 
       * // Normalize the alpha mask to keep intensity between 0 and 1 alpha.convertTo(alpha, CV_32FC3, 1.0/255); //
       * 
       * // Storage for output image Mat ouImage = Mat::zeros(mask.size(), mask.type());
       * 
       * // Multiply the mask with the alpha matte multiply(alpha, mask, mask);
       * 
       * // Multiply the background with ( 1 - alpha ) multiply(Scalar::all(1.0)-alpha, background, background);
       * 
  ...
  /*
   * void overlayImage(final Mat background, final Mat foreground, Mat output, int posX, int posY){
   * 
   * background.copyTo(output);
   * 
   * BytePointer bbp = background.data(); BytePointer fbp = foreground.data(); bbp.asByteBuffer();
   * 
   * // start at the row indicated by location, or at row 0 if posY is negative. for(int y = Math.max(posY , 0); y <
   * background.rows(); ++y) { int fY = y - posY; // because of the translation
   * 
   * // we are done of we have processed all rows of the foreground image. if(fY >= foreground.rows()) break;
   * 
   * // start at the column indicated by location,
   * 
   * // or at column 0 if posX is negative. for(int x = Math.max(posX, 0); x < background.cols(); ++x) { int fX = x - posX; //
   ...
   * 
   * 
   * // determine the opacity of the foregrond pixel, using its fourth (alpha) channel. double opacity = ((double)foreground.data[fY
   ...
   * 
   * // but only if opacity > 0. for(int c = 0; opacity > 0 && c < output.channels(); ++c) { char foregroundPx = foreground.data[fY
   * * foreground.step() + fX * foreground.channels() + c]; char backgroundPx = background.data[y * background.step() + x *
   * background.channels() + c]; output.data[y*output.step + output.channels()*x + c] = backgroundPx * (1.-opacity) + foregroundPx *
   * opacity; } } } }
   * 
   */
