...
public class OpenCVFilterAddMask extends OpenCVFilter {
  ...
  void overlayImage(final Mat background, final Mat foreground, Mat output, int posX, int posY){
    ...
    BytePointer bbp = background.data();
    BytePointer fbp = foreground.data();
    bbp.asByteBuffer();
    
      // start at the row indicated by location, or at row 0 if posY is negative.
      for(int y = Math.max(posY , 0); y < background.rows(); ++y)
      {
        int fY = y - posY; // because of the translation
        ...
        // or at column 0 if posX is negative.
        for(int x = Math.max(posX, 0); x < background.cols(); ++x)
        {
          int fX = x - posX; // because of the translation.
          ...
          double opacity =
            ((double)foreground.data[fY * (int)foreground.step() + fX * foreground.channels() + 3])
