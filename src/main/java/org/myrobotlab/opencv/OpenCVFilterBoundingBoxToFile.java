	...
	public IplImage process(IplImage image) {
		...
		if (bb != null) {
			for (int i = 0; i < bb.size(); ++i) {
				...
				IplImage copy = cvCreateImage(new CvSize((int) r.width, (int) r.height), image.depth(),
						image.nChannels());
				cvCopy(image, copy, null); // roi vs mask ?

				saveToFile(String.format("%s" + File.separator + "%07d-%03d.png", targetDir, opencv.getFrameIndex(), i),
						copy);
