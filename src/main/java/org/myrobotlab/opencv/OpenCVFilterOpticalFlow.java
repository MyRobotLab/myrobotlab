...
public class OpenCVFilterOpticalFlow extends OpenCVFilter {
	...
	/**
	 * Optional region of interest. If the image is not empty (it needs to have the
	 * type CV_8UC1 and the same size as image ), it specifies the region in which
	...
	public IplImage process(IplImage inImage) throws InterruptedException {
		...
		if (lastImg != null) {
			...
			cvGoodFeaturesToTrack(currentImg, eig_image, tmpImage, corners, maxCorners, qualityLevel, minDistance, roi,
					blockSize, useHarrisDetector, harrisDetectorK);

			if (getCornerSubPix) {
				cvFindCornerSubPix(currentImg, corners, maxCorners.get(), cvSize(winSize, winSize), cvSize(-1, -1),
			...
			cvCalcOpticalFlowPyrLK(currentImg, lastImg, pyrA, pyrB, corners, cornersB, maxCorners.get(),
