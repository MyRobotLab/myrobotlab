	...
	private void displayResult(IplImage image, ArrayList<YoloDetectedObject> result) {
		...
		for (YoloDetectedObject obj : result) {
			...
			if (!StringUtils.isEmpty(subLabel)) {
				cvPutText(image, subLabel, cvPoint(obj.boundingBox.x(), obj.boundingBox.y() + 20), font,
						CvScalar.YELLOW);
			}
		...
		for (int i = 0; i < detectionMat.rows(); i++) {
			...
			for (int c = CONFIDENCE_INDEX + 1; c < currentRow.size().get(); c++) {
				...
				if (val > 0.0) {
					...
					log.info(label + " (" + confidence + "%) [(" + xLeftBottom + "," + yLeftBottom + "),(" + xRightTop
							+ "," + yRightTop + ")]");
					...
					YoloDetectedObject obj = new YoloDetectedObject(boundingBox, confidence, label,
							getOpenCV().getFrameIndex(), cropped, null);
					yoloObjects.add(obj);
					...
					if (obj.label.equals("person")) {
						if (dl4j != null && personModel != null) {
							...
							try {
								dl4JResult = dl4j.classifyImageCustom(cropped, personModel.getModel(),
										personModel.getLabels());
							} catch (IOException e) {
								// TODO Auto-generated catch block
