...
public class OpenCVFilterYolo extends OpenCVFilter implements Runnable {
	...
	private void loadYolo() {
		...
		net = readNetFromDarknet(darknetHome + File.separator + modelConfig,
				darknetHome + File.separator + modelWeights);
	 ...
	 * 
	 * 		cvDrawRect(image, leftCorner,
	 * 				cvPoint(obj.boundingBox.x() + 30 * obj.label.length(), obj.boundingBox.y() - 20), CvScalar.BLUE,
	 * 				CV_FILLED, 0, 0);
	 * 		// TODO - hashcode related color
	 * 		// CvScalar objColor = cvColorToScalar(obj.label.hashCode(), CV_8U);
	 * 		cvPutText(image, label, cvPoint(obj.boundingBox.x() + 6, obj.boundingBox.y() - 6), font,
	 * 				CvScalar.YELLOW);
	 ...
	 * 	cvDrawRect(image, cvPoint(rect.x(), rect.y()), cvPoint(rect.x() + rect.width(), rect.y() + rect.height()),
	 * 			color, 1, 1, 0);
	 * }
	 * </pre>
	 */
		...
		for (int i = 0; i < detectionMat.rows(); i++) {
			...
			for (int c = CONFIDENCE_INDEX + 1; c < currentRow.size().get(); c++) {
				...
				if (val > 0.0) {
					...
					log.debug(label + " (" + confidence + "%) [(" + xLeftBottom + "," + yLeftBottom + "),(" + xRightTop
							+ "," + yRightTop + ")]");
					Rect boundingBox = new Rect(xLeftBottom, yLeftBottom, xRightTop - xLeftBottom,
					...
					Classification obj = new Classification(
							String.format("%s.%s-%d", data.getName(), name, data.getFrameIndex()));
					obj.setLabel(label);
