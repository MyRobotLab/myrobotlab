	...
	public Object process(BufferedImage output, BufferedImage image) {
		...
		for (int y = 0; y < yTotal; ++y) {
			for (int x = 0; x < xTotal; ++x) {
				...
				/*
				 * if (lastColor != colorInt) { log.info("group size " + groupList.size() +
				 * " @ (" + x + "," + y + ") color red " + ((colorInt >> 16) & 0xFF) + " green "
				 * + ((colorInt >> 8) & 0xFF) + " blue " + ((colorInt) & 0xFF)); for (int i = 0;
				 * i < groupList.size(); ++i) { Group group = groupList.get(i);
				 * log.info("number " + group.number); log.info("color red " + ((group.intColor
				 * >> 16) & 0xFF) + " green " + ((group.intColor >> 8) & 0xFF) + " blue " +
				 * ((group.intColor) & 0xFF)); log.info("bounding box " + group.boundingBox);
				...
				if (y != 0 && x != 0) {
					...
					if (Math.abs(((ncolorInt >> 16) & 0xFF) - red) < redGroupDelta
						...
						if ((isGrouped) && myGroup != neighborGroup) {
							...
							if (neighborGroup.boundingBox.x + neighborGroup.boundingBox.width > myGroup.boundingBox.x
							...
							if (neighborGroup.boundingBox.y + neighborGroup.boundingBox.height > myGroup.boundingBox.y
