...
public abstract class OpenCVFilter implements Serializable {
	...
	static public IplImage loadImage(String infile) {
		...
		tryfile = "src" + File.separator + "main" + File.separator + "resources" + File.separator + "resource"
				+ File.separator + OpenCV.class.getSimpleName() + File.separator + infile;
		f = new File(tryfile);
		if (f.exists()) {
			return load(tryfile);
		} else {
		...
		tryfile = "src" + File.separator + "test" + File.separator + "resources" + File.separator
				+ OpenCV.class.getSimpleName() + File.separator + infile;
		f = new File(tryfile);
		if (f.exists()) {
			return load(tryfile);
		} else {
	...
	/**
	 * This allows the display method to be processed in the filter typically its a
	 * conversion from opencv-jni-land to java-land and associated processing for
	 * human consumption
	 */
	...
	/**
	 * This is NOT the filter's image, but really the output of the previous filter
	 * ! to be used as input for "this" filters process method
	 * 
	 * @return
	 */
	...
	/**
	 * method which determines if this filter to process its display TODO - have it
	 * also decide if its cumulative display or not
	 */
