 ...
 *
 */
public class OpenCVFilterFaceRecognizer2 extends OpenCVFilter implements Runnable {
  ...
  private String cascadeDir = Util.getResourceDir() + File.separator + "OpenCV/haarcascades";
  ...
  public OpenCVFilterFaceRecognizer2(String filterName, String sourceKey) {
  ...
  public void init() {
    ...
    Thread recognizerThread = new Thread(this, "FaceRecognitionThread");
    recognizerThread.start();
    log.info("Face Recognition Thread Started : {}", this.name);
  ...
  public void run() {
    ...
    while (true) {
      ...
      Frame grayFrame = makeGrayScale(lastImage);
      ...
      if (Mode.TRAIN.equals(mode)) {
        ...
        cvPutText(lastImage, status, cvPoint(20, 40), font, CvScalar.GREEN);
      } else if (Mode.RECOGNIZE.equals(mode)) {
        String status = "Recognize Mode:" + lastRecognizedName;
        cvPutText(lastImage, status, cvPoint(20, 40), font, CvScalar.YELLOW);
      }
        ...
        if (dF.isComplete()) {
          ...
          if (Mode.TRAIN.equals(mode)) {
            ...
            if (!StringUtils.isEmpty(trainName)) {
              ...
              cvPutText(lastImage, "Snapshot Saved: " + trainName, cvPoint(20, 60), font, CvScalar.CYAN);
            }
          } else if (Mode.RECOGNIZE.equals(mode)) {
            ...
            if (!trained) {
              ...
              {
                cvPutText(lastImage, "[unicode char detected !!! text not visible, but succesfully parsed !]", cvPoint(5, 60), fontWarning, CvScalar.RED);
              }
              cvPutText(lastImage, name, dF.resolveGlobalLowerLeftCorner(), font, CvScalar.CYAN);
