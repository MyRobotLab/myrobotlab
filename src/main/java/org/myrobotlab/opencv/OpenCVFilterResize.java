...
public class OpenCVFilterResize extends OpenCVFilter {
	 ...
	 * 
	 * // find the dimension (w or h) where when the original image is scaled // the
	 * first dimension which "fits" determines the percentage of what // the image
	 * should scale to int dw = Math.abs(maxWidth - img.width()); int dh =
