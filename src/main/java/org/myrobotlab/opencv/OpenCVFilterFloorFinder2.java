		...
		while (contours != null && !contours.isNull()) {
			if (contours.elem_size() > 0) { // TODO - limit here for
				...
				if (isMinArea && isMaxArea) {
					...
					/*
					 * WRONG FIXME - post processing should be done in Java on the buffered image
					 * !!!!S cvPutText(display, " " + points.total() + " " + (rect.x() +
					 * rect.width() / 2) + "," + (rect.y() + rect.height() / 2) + " " + rect.width()
					 * + "x" + rect.height() + "=" + (rect.width() * rect.height()) + " " + " " +
					 * cvCheckContourConvexity(points), cvPoint(rect.x() + rect.width() / 2,
					 * rect.y()), font, CvScalar.WHITE);
					 */
				}
