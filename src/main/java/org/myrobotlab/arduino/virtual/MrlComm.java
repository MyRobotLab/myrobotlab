...
public class MrlComm {
	 ...
	 * void processCommand(); void processCommand(int ioType); void updateDevices();
	 * unsigned int getCustomMsg(); int getCustomMsgSize(); void
	...
	private int analogRead(int address) {
		/*
		 * to simulate longer analogReads :) try{ Thread.sleep(45); } catch(Exception
		 * e){}
		 */
	 ...
	 * 
	 * @param id - the device id to fetch. (I think this is the internal device id
	 ...
	 * Id leaves something to be desired - and the "index" does not spin through the
	 * deviceList to find it .. a dynamic array of pointers would only expand if it
	 * could not accomidate the current number of devices, when a device was removed
	...
	/***********************************************************************
	 * PUBLISH_BOARD_INFO This function updates the average time it took to run the
	 * main loop and reports it back with a publishBoardStatus MRLComm message
	 *
	...
	public void publishBoardInfo() {
		...
		msg.publishBoardInfo(MRLCOMM_VERSION, Arduino.BOARD_TYPE_ID_UNO, load, getFreeRam(), pinList.size(),
				deviceSummary);
		lastBoardInfoUs = now;
		loopCount = 0;
	}
