...
public class InMoov extends Service {
	...
	public void startHeadTracking(String port, Integer rothead, Integer neck) {
		log.warn(
				"Please use ServoControl : startHeadTracking(YourServoRothead,YourServoNeck), I will try to do it for you...");
		startHeadTracking();
	}
	...
	public InMoovHead startHead(String port, Integer headYPin, Integer headXPin, Integer eyeXPin, Integer eyeYPin,
			Integer jawPin, Integer rollNeckPin) throws Exception {
		return startHead(port, null, headYPin, headXPin, eyeXPin, eyeYPin, jawPin, rollNeckPin);
	}

	public InMoovHead startHead(String port, String type, Integer headYPin, Integer headXPin, Integer eyeXPin,
			Integer eyeYPin, Integer jawPin, Integer rollNeckPin) throws Exception {
	 ...
	 */
	public InMoovEyelids startEyelids(ServoController controller, Integer eyeLidLeftPin, Integer eyeLidRightPin)
			throws Exception {
	...
	public void publishPin(Pin pin) {
		...
		if (pirPin == pin.pin && startSleep != null && pin.value == 1) {
			 ...
			 * (now.get(Calendar.HOUR_OF_DAY) < 12) { salutation = "good morning "; } else
			 * if (now.get(Calendar.HOUR_OF_DAY) < 16) { salutation = "good afternoon "; }
	...
	public MouthControl startMouthControl(String port) {
		log.warn(
				"Please use ServoControl : startMouthControl(YourServoJaw,mouthService), I will try to do it for you...");
		...
		try {
			...
			for (ServiceInterface service : Runtime.getServices()) {
				if (service instanceof Servo) {
					...
					// save the servo map
					calibrationWriter.write(s.getName() + ".map(" + s.getMinInput() + "," + s.getMaxInput() + ","
							+ s.getMinOutput() + "," + s.getMaxOutput() + ")\n");
					// if there's a controller reattach it at rest
					if (s.getController() != null) {
						String controller = s.getController().getName();
						calibrationWriter.write(s.getName() + ".attach(\"" + controller + "\"," + s.getPin() + ","
								+ s.getRest() + ")\n");
					}
					if (s.getAutoDisable()) {
