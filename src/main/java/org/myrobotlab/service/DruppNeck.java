...
public class DruppNeck extends Service {
	...
	public void moveTo(double roll, double pitch, double yaw) throws Exception {
		...
		// Ok, servos can only (typically) move from 0 to 180.. if any of the angles are
		// negative... we can't move there.. let's log a warning
		// TODO: use the actual min/max .. and if we're out of range.. then log this.
		...
		if (upDeg < 0 || middleDeg < 0 || downDeg < 0 || upDeg > 180 || middleDeg > 180 || downDeg > 180) {
			log.warn("Target Position out of range! {} Pitch {} Yaw {} -> Up {} Middle {} Down {}", roll, pitch, yaw,
					MathUtils.round(upDeg, 3), MathUtils.round(middleDeg, 3), MathUtils.round(downDeg, 3));
			// Skipping this movement as it's likely unstable!
			return;
		}
		log.info("Input Roll {} Pitch {} Yaw {} -> Up {} Middle {} Down {}", roll, pitch, yaw,
				MathUtils.round(upDeg, 3), MathUtils.round(middleDeg, 3), MathUtils.round(downDeg, 3));
