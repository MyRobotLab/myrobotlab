...
public class PythonProxy extends Service {
	...
	public boolean preProcessHook(Message msg) {
		...
		if (connected) {
			...
			log.error(
					"Proxy is not connected and can't send message to service. Check to see if native service was started.");
			return false;
		}
	}
		...
		if (os.toLowerCase().equals("linux")) {
			try {
				if (!startedNativeService) {
					...
					Process p = new ProcessBuilder("xterm", "-e",
							"./native-mrl-services/utils/bin/virtualenv --relocatable native-mrl-services/python && . native-mrl-services/python/bin/activate && pip install -r native-mrl-services/python/requirements.txt && python native-mrl-services/python/services/"
