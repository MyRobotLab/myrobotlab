...
public class Adafruit16CServoDriver extends Service implements I2CControl, ServoController, MotorController {
	...
	public class SpeedControl extends Thread {
		...
		public void run() {
			...
			servoData = servoMap.get(name);
			log.debug("Moving from {} to {} at {} degrees/second", servoData.currentOutput, servoData.targetOutput,
					servoData.velocity);
			...
			try {
				...
				while (servoData.isMoving && servoData.isEnergized) {
					...
					int pulseWidthOff = SERVOMIN + (int) (servoData.currentOutput
							* (int) ((float) SERVOMAX - (float) SERVOMIN) / (float) (180));
				...
				log.info("publishServoEvent : {} , event {}, currentOutput {}", servoData.servo.getName(), 1,
						servoData.currentOutput);
	...
	public double publishServoEvent(ServoControl servo, Integer eventType, double currentOutput) {
		...
		servo.onServoEvent(eventType, currentOutput);
	...
	/**
	 * @Mats - added by GroG - was wondering if this would help, probably you need a
	 *       reverse index too ?
	...
	void setServo(Integer pin, Integer pulseWidthOff) {
		...
		// sent as 2 bytes
		/*
		 * log.debug( String.format("setServo %s deviceAddress %s pin %s pulse %s", pin,
	...
	/**
	 * this would have been nice to have Java 8 and a default implementation in this
	 * interface which does Servo sweeping in the Servo (already implemented) and
	 * only if the controller can does it do sweeping on the "controller"
	 * 
	...
	public void servoMoveTo(ServoControl servo) {
		...
		if (servoData.isEnergized) {
			...
			if (servoData.velocity == -1) {
				...
				log.debug("servoWrite {} deviceAddress {} targetOutput {}", servo.getName(), deviceAddress,
	...
	public void servoWriteMicroseconds(ServoControl servo, int uS) {
		...
		// since pulseWidthOff can be larger than > 256 it needs to be
		// sent as 2 bytes
		log.debug("servoWriteMicroseconds {} deviceAddress {} pin {} pulse {}", servo.getName(), deviceAddress, pin,
				pulseWidthOff);
	...
	/*
	 * @Override public boolean isAttached(String name) { return (controller != null
	 * && controller.getName().equals(name) || servoMap.containsKey(name)); }
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.myrobotlab.service.interfaces.ServoController#disablePin(java.lang.
	 * Integer)
	 */
	@Override
