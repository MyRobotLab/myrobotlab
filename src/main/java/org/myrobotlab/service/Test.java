...
public class Test extends Service implements StatusListener {
	...
	public static class TestData implements Serializable {
		...
		public String toString() {
			return String.format("=== TEST ===> testName: %s service: %s/%s status: %s", testName, branch, serviceName,
					status);
		}
	}
	...
	public void generatePythonScripts(String branch) {
		try {
			...
			for (String service : services) {
				...
				/*
				 * t.append("###########################################"); t.append("# " +
				 * service + ".py service python script\n"); t.append("# more information @\n");
				...
				t.append(String.format("# categories: %s\n",
						Arrays.toString(st.categories.toArray(new String[st.categories.size()])).replace("[", "")
								.replaceAll("]", "")));
	...
	public Map<String, String> getPyRobotLabServiceScripts(String branch) throws Exception {
		...
		if (pythonScripts == null) {
			...
			log.info("remove scripts - script found but no service type {}",
					matrix.currentProgress.pythonScriptsWithNoServiceType);
	 ...
	 * 
	 * public Status pythonTest() { Python python = (Python) Runtime.start("python",
	 * "Python"); Serial uart99 = (Serial) Runtime.start("uart99", "Serial"); //
	 * take inventory of currently running services HashSet<String> keepMeRunning =
	 * new HashSet<String>();
	 * 
	 ...
	 * removed). And all services which do not have scripts (to be added) It runs in
	 * the same process as Python and the expectation is the Agent (with the help of
	 * the Test service) has created an environment where the service to be tested
	...
	public TestData testJUnit(TestData test) {
		...
		try {
			...
			/*
			 * Suite suite = new Suite(klass, new RunnerBuilder() { ... // Implement methods
			 * }); JUnitCore c = new JUnitCore(); c.run(Request.runner(suite));
			 */
			 ...
			 * result.status = Status.error("script not found"); } else { result.status =
			 * Status.success(); }
			 */
	...
	public TestData testPythonScript(TestData test) throws Exception {
		...
		for (String cs : currentServices) {
			if (!preServices.contains(cs)) {
				log.info("service {} created for testing {} - test finished attempting to release", cs,
						test.serviceName);
	...
	public TestData testPythonScriptExists(TestData test) {
		...
		try {
			...
			String url = String.format("https://github.com/MyRobotLab/pyrobotlab/edit/%s/service/%s.py", test.branch,
					name);
			...
			if (script == null) {
				 ...
				 * sd.getServiceType(test.fullTypeName); StringBuffer t = new StringBuffer();
				 * t.append("#########################################\n");
				 * t.append(String.format("# %s.py\n", name));
				 * t.append(String.format("# description: %s\n", st.getDescription()));
				 * t.append(String.format("# categories: %s\n",
				 ...
				 * t.append(String.format("%s = Runtime.start(\"%s\",\"%s\")", lowercase,
				 * lowercase, name)); FileIO.toFile(new File(String.format("%s.py", name)),
				 * t.toString().getBytes());
				 */
			} else {
				test.status = Status.success();
