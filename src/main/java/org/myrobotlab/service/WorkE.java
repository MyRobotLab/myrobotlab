...
public class WorkE extends Service implements StatusListener {
	...
	public void startFlow() {
		...
		/*
		 * // Insert points into subdiv (points is a vector<cv::Point2f>) for (size_t i
		 * = 0; i < points.size(); ++i) subdiv.insert(points[i]);
		 * 
	...
	public static void main(String[] args) {
		try {
			...
			/*
			 * ProgramAB brain = worke.getBrain(); // FIXME - fix for 2 lines create and
			 * getResponse - use null brain.setCurrentBotName("worke"); // FIXME - scan
			 * directory for bots brain.startSession("default", "worke");
