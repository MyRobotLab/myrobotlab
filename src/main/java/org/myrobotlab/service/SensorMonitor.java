...
public class SensorMonitor extends Service {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		try {
			...
			/*
			 * 
			 * Random rand = new Random(); for (int i = 0; i < 10000; ++i) { Message msg =
			 * new Message(); msg.name="sensors"; msg.sender="SEAR"; msg.method="input";
			 * Float[] gps = new Float[]{rand.nextFloat()*200, rand.nextFloat()*200,
			 * rand.nextFloat()*200}; msg.data = new Object[]{gps}; //msg.data = new
			 * Object[]{rand.nextFloat()*200}; sm.in(msg); }
			 */
		} catch (Exception e) {
			Logging.logError(e);
		}
	...
	public final void addTrigger(Trigger trigger) {
		if (trigger.pinData.source == null) {
			log.error("addTrigger adding trigger with no source controller - will be based on pin only ! "
					+ trigger.pinData.pin);
		}
