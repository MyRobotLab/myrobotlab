...
public class Lidar extends Service implements SerialDataListener {
	...
	public Integer onByte(Integer b) throws IOException {
		...
		if (log.isDebugEnabled()) {
			log.debug(String.format("byteReceived Index = %d expected message size = %d data = %02x", index,
					dataMessageSize, b));
		}
	...
	public int[] publishLidarData() {
		...
		log.info("publishLidarData has been called. Message length = " + message.length + "        We should have = "
				+ intData.length + "data readings");
		...
		// number
		{
			...
			log.info("IntData index = " + (i - 8) / 2 + " i = " + i + " message = "
					+ String.format(" %02x %02x", message[i], message[i + 1])
	...
	public void setBaud(int baudRate) throws Exception {
		...
		/*
		 * 9600 is default, but just in case you ever need it... PC sends : 02 00 02 00
		 * 20 42 52 08 LMS replies: 06 02 81 03 00 A0 00 10 36 1A (success)
		 */
		if (baudRate == 9600) {
			serial.write(new byte[] { 0x02, 0x00, 0x02, 0x00, 0x20, 0x42, 0x52, 0x08 });
		} /*
			 * 19200 PC sends : 02 00 02 00 20 41 51 08 LMS replies: 06 02 81 03 00 A0 00 10
			 * 36 1A (success)
			 */else if (baudRate == 19200) {
			serial.write(new byte[] { 0x02, 0x00, 0x02, 0x00, 0x20, 0x41, 0x52, 0x08 });
		} /*
			 * 38400 PC sends : 02 00 02 00 20 41 51 08 LMS replies: 06 02 81 03 00 A0 00 10
			 * 36 1A (success)
	...
	public void setScanMode(int spread, float angularResolution) throws Exception {
		...
		if (spread == 180) {
			if (angularResolution == 1) {
				serial.write(
						new byte[] { 0x02, 0x00, 0x05, 0x00, 0x3B, (byte) 0xB4, 0x00, 0x64, 0x00, (byte) 0x97, 0x49 });
