...
public class Servo extends Service implements ServoControl {
	...
	public class Sweeper extends Thread {
		 ...
		 * and input sweeps back and forth - the servo parameters know what to do for
		 * output
		 */
	...
	/**
	 * the 'pin' for this Servo - it is Integer because it can be in a state of 'not
	 * set' or null.
	 * 
	...
	/**
	 * feedback of both incremental position and stops. would allow blocking moveTo
	 * if desired
	 */
	 ...
	 * 
	 * @param defaultDisableDelayNoVelocity - milliSeconds
	 * @default - 10000
	 */
	 ...
	 * 
	 * @param disableDelayIfVelocity - milliSeconds
	 * @default - 1000
	 */
	...
	/**
	 * This method will leave the servo connected to the controller, however it will
	 * stop sending pwm messages to the servo.
	 */
	@Override
	public void disable() {
		enabled = false;
	...
	public void map(double minX, double maxX, double minY, double maxY) {
		...
		if (minX != mapper.getMinX() || maxX != mapper.getMaxX() || minY != mapper.getMinY()
				|| maxY != mapper.getMaxY()) {
	 ...
	 * standard servos a parameter value of 1000 is fully counter-clockwise, 2000 is
	 * fully clockwise, and 1500 is in the middle.
	 * 
	 * Note that some manufactures do not follow this standard very closely so that
	 * servos often respond to values between 700 and 2300. Feel free to increase
	 * these endpoints until the servo no longer continues to increase its range.
	 * Note however that attempting to drive a servo past its endpoints (often
	 * indicated by a growling sound) is a high-current state, and should be
	 * avoided.
	 * 
	...
	/**
	 * The attachServoController method is used to "attach" 2 services together, the
	 * ServoControl & the ServoController. All the necessary data needed to attach
	 * this Servo to the servo controller should be available. First we check to see
	 * if this controller is already attached. If it is we are done. If not we
	 * proceed to attach the controller and at the end of the function we call the
	 * controller.attach(this) to give the controller an opportunity to do its
	 * attach logic.
	 * 
	...
	/*
	 * Deprecated enableAutoAttach will attach a servo when ask to move and it when
	...
	public void publishedSensorEventFeedback(PinData[] data) {
		...
		if (targetPosBeforeSensorFeebBackCorrection > 0 && MathUtils.averageMaxFromArray(5, sensorValues) > MathUtils
				.getPercentFromRange(autoCalibrateMin, autoCalibrateMax, this.servoTorque)) {
		...
		if (targetPosBeforeSensorFeebBackCorrection < 180 && MathUtils.averageMaxFromArray(5, sensorValues) < MathUtils
				.getPercentFromRange(autoCalibrateMin, autoCalibrateMax, this.servoTorque)) {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		Adafruit16CServoDriver adafruit16CServoDriver = (Adafruit16CServoDriver) Runtime.start("adafruit16CServoDriver",
	...
	public void saveCalibration(String calibrationFilename) {
		...
		try {
			...
			// save the servo map
			calibrationWriter.write(this.getName() + ".map(" + this.getMinInput() + "," + this.getMaxInput() + ","
					+ this.getMinOutput() + "," + this.getMaxOutput() + ")\n");
			// if there's a controller reattach it at rest
			if (this.getController() != null) {
				String controller = this.getController().getName();
				calibrationWriter.write(this.getName() + ".attach(\"" + controller + "\"," + this.getPin() + ","
						+ this.getRest() + ")\n");
			}
