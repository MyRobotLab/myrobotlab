...
public class OledSsd1306 extends Service implements I2CControl {
	 ...
	 * 
	 * Note that the current buffer is not cleared before, so if you want the image
	 * to completely overwrite the current display content you need to call clear()
	 * before.
	 *
	 */
	...
	void ssd1306_command(int c) {
		...
		controller.i2cWrite((I2CControl) this, Integer.parseInt(deviceBus), Integer.decode(deviceAddress), buffer,
				buffer.length);
	}
	...
	public void display() {
		...
		for (int i = 0; i < (SSD1306_LCDWIDTH * SSD1306_LCDHEIGHT / 8) - 1; i++) {
			...
			controller.i2cWrite((I2CControl) this, Integer.parseInt(deviceBus), Integer.decode(deviceAddress),
