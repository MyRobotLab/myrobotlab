...
public interface SerialDevice extends PortPublisher, Attachable {
  ...
  /*
   * FIXME - make like http://pyserial.sourceforge.net/pyserial_api.html with blocking &amp; timeout InputStream like interface -
   * but regrettably InputStream IS NOT A F#(@!! INTERFACE !!!!
   * 
   * WORTHLESS INPUTSTREAM FUNCTION !! -- because if the size of the buffer is ever bigger than the read and no end of stream has
   * occurred it will block forever :P
   * 
