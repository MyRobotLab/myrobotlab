...
public interface ServoControl extends AbsolutePositionControl, Attachable, MessageSubscriber {
	...
	/**
	 * @param degreesPerSecond degrees per second rotational velocity cm per second
	 *                         linear velocity ?
	 * 
	 */
	 ...
	 * controller has - updates the Servo's pin and returns the refreshed data. Not
	 * worth it. What will happen is the pin which was set on the servo will simply
	 * be returned
	 * 
	...
	/**
	 * Unmapped current position of last input. ie. equivalent to the last position
	 * from moveTo(pos)
	 * 
	 * A possible better solution might be to use ServoEvent(s) to get the position
	 ...
	 * controller needs a stable method &amp; a stable unit FIXME - not sure if this
	 * is a good thing to expose
	 * 
	...
	/**
	 * set the pin of the servo this does not 'attach' energize the pin only set the
	 * pin value
	 * 
	...
	/**
	 * @param rest A default position for the servo. Defaulted to 90 unless
	 ...
	 * 180 a global moveToBlocking is a method that use multiple servo at same time
	 ...
	 * gestures that leave your arms in the air ... so if overrideautoDisable(true)
	 * servo will never autoDisable until overrideautoDisable(false) ( we need to
	 * keep original autoDisable status, that is the reason )
	 */
