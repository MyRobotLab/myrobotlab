...
public class WiiDar extends Service {
	 ...
	 * multiple methods) IRData and Servo Data. The synchronization can be software
	 * technique ie. pausing the system until a the servo command to move to a
	 * location has propagated to an IREvent. Or, preferably it would be a hardware
	 * event where an motor's encoder data is paired with the appropriate IREvent.
	 */
	 ...
	 * lastEncoderData = null; arduino.pinMode(4, Arduino.INPUT); // set the encoder
	 * to input mode TODO - this could be bug worthy
	 * 
	 * for (int i = 60; i < 180; ++i) { servo.moveTo(i); Thread.sleep(30); if
	 * (lastIRData != null) { invoke("setServoRightMax", i); invoke("setIRRightMax",
	 * lastIRData); break; } }
	 * 
	 ...
	 * System.currentTimeMillis()); servo.moveTo(i); Thread.sleep(servoPause); p.ir
	 * = lastIRData; points.add(p); }
	 * 
