...
public class EddieControlBoard extends Service implements KeyListener, SerialDataListener, JoystickListener {
	...
	public static void main(String[] args) {
		...
		try {
			 ...
			 * float i = 0.94f;
			 * 
			 * Map mapper = new Map(-1.0f, 1.0f, -127.0f, 127.0f); int x = mapper.calc(i);
			 * if (x > 127) { x = 128 - x; }
			 * 
			 * log.info("{}", Integer.toHexString(x & 0xFF));
			 * 
			 * String hex = Integer.toHexString(256 & 0xFF); log.info(hex.toUpperCase());
			 * hex = Integer.toHexString(255 & 0xFF); log.info(hex.toUpperCase()); hex =
			 * Integer.toHexString(230 & 0xFF); // slow reverse log.info(hex.toUpperCase());
			 */
	...
	/**
	 * sending a command when expecting a string response in the context of blocking
	 * for response
	 * 
	 * @param cmd to send
