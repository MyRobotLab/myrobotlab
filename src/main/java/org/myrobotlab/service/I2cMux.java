...
public class I2cMux extends Service implements I2CControl, I2CController {
	...
	public List<String> deviceAddressList = Arrays.asList("0x70", "0x71", "0x72", "0x73", "0x74", "0x75", "0x76",
			"0x77");
	...
	public void setMuxBus(int busAddress) {
		if (busAddress != lastBusAddress) {
			...
			log.debug("setMux this.deviceBus {} this.deviceAddress {} bus[0] {}", this.deviceBus, this.deviceAddress,
					bus[0]);
			controller.i2cWrite(this, Integer.parseInt(this.deviceBus), Integer.decode(this.deviceAddress), bus,
					bus.length);
	...
	public void i2cWrite(I2CControl control, int busAddress, int deviceAddress, byte[] buffer, int size) {
		...
		log.debug(
				String.format("i2cWrite busAddress x%02X deviceAddress x%02X key %s", busAddress, deviceAddress, key));
		controller.i2cWrite(this, Integer.parseInt(this.deviceBus), deviceAddress, buffer, size);
	}

	/**
	 * TODO Add demuxing. i.e the route back to the caller The i2c will receive data
	 * that neeeds to be returned syncronous or asycncronus
	 */
	@Override
	...
	public int i2cWriteRead(I2CControl control, int busAddress, int deviceAddress, byte[] writeBuffer, int writeSize,
		...
		setMuxBus(busAddress);
		controller.i2cWriteRead(this, Integer.parseInt(this.deviceBus), deviceAddress, writeBuffer, writeSize,
				readBuffer, readSize);
