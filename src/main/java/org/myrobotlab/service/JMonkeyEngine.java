...
public class JMonkeyEngine extends Service {
	...
	public void simpleInitApp() {
		...
		putNode("i01.torso.lowStom", "rootNode", "Models/ltorso.j3o", null, Vector3f.UNIT_X.mult(1),
				new Vector3f(0, 0, 0), 0);

		// FIXME - bind this to process Peers !!!! "default" "Peer" info in 3D form

		putNode("i01.torso.midStom", "i01.torso.lowStom", "Models/mtorso.j3o", new Mapper(0, 180, 120, 60),
				Vector3f.UNIT_Y.mult(-1), new Vector3f(0, 0, 0), 0);
		putNode("i01.torso.topStom", "i01.torso.midStom", "Models/ttorso1.j3o", new Mapper(0, 180, 80, 100),
				Vector3f.UNIT_Z.mult(1), new Vector3f(0, 105, 10), 0);
		...
		putNode("i01.rightArm.omoplate", "rightS", "Models/Romoplate1.j3o", new Mapper(0, 180, 10, 70),
				Vector3f.UNIT_Z.mult(-1), new Vector3f(-143, 0, -17), 0);

		// angle = rotationMask.mult((float) Math.toRadians(-2));
		// node.rotate(angle.x, angle.y, angle.z); <------------ additional rotation
		// ...
		putNode("i01.rightArm.shoulder", "i01.rightArm.omoplate", "Models/Rshoulder1.j3o", new Mapper(0, 180, 0, 180),
				Vector3f.UNIT_X.mult(-1), new Vector3f(-23, -45, 0), 0);
		putNode("i01.rightArm.rotate", "i01.rightArm.shoulder", "Models/rotate1.j3o", new Mapper(0, 180, 40, 180),
		...
		putNode("i01.rightArm.bicep", "i01.rightArm.rotate", "Models/Rbicep1.j3o", new Mapper(0, 180, 5, 60),
				Vector3f.UNIT_X.mult(-1), new Vector3f(5, -225, -32), 0);
		putNode("leftS", "i01.torso.topStom", "Models/Lomoplate1.j3o", null, Vector3f.UNIT_Z.mult(1),
				new Vector3f(0, 300, 0), 0);

		// angle = rotationMask.mult((float) Math.toRadians(4));
		// node.rotate(angle.x, angle.y, angle.z); <-- another rotation ...
		putNode("i01.leftArm.omoplate", "leftS", "Models/Lomoplate1.j3o", new Mapper(0, 180, 10, 70),
				Vector3f.UNIT_Z.mult(1), new Vector3f(143, 0, -15), 0);
		putNode("i01.leftArm.shoulder", "i01.leftArm.omoplate", "Models/Lshoulder.j3o", new Mapper(0, 180, 0, 180),
				Vector3f.UNIT_X.mult(-1), new Vector3f(17, -45, 5), 0);
		putNode("i01.leftArm.rotate", "i01.leftArm.shoulder", "Models/rotate1.j3o", new Mapper(0, 180, 40, 180),
		...
		// !!!
		putNode("i01.leftArm.bicep", "i01.leftArm.rotate", "Models/Lbicep.j3o", new Mapper(0, 180, 5, 60),
				Vector3f.UNIT_X.mult(-1), new Vector3f(-14, -223, -28), 0);

		// angle = rotationMask.mult((float) Math.toRadians(-90)); <- Ha .. and an
		// additional rotate
		// node.rotate(angle.x, angle.y, angle.z);
		putNode("i01.rightHand.wrist", "i01.rightArm.bicep", "Models/RWristFinger.j3o", new Mapper(0, 180, 130, 40),
				Vector3f.UNIT_X.mult(-1), new Vector3f(15, -290, -10), 0);

		// angle = rotationMask.mult((float) Math.toRadians(-90)); <--- HA .. and
		// additional rotation !
		// node.rotate(angle.x, angle.y, angle.z);
		putNode("i01.leftHand.wrist", "i01.leftArm.bicep", "Models/LWristFinger.j3o", new Mapper(0, 180, 40, 130),
				Vector3f.UNIT_Y.mult(1), new Vector3f(0, -290, -20), 90);
		putNode("i01.head.neck", "i01.torso.topStom", "Models/neck.j3o", new Mapper(0, 180, 60, 110),
				Vector3f.UNIT_X.mult(-1), new Vector3f(0, 452.5f, -45), 0);
		putNode("i01.head.rollNeck", "i01.head.neck", null, new Mapper(0, 180, 60, 115), Vector3f.UNIT_Z.mult(1),
				new Vector3f(0, 0, 0), 90);
		putNode("i01.head.rothead", "i01.head.rollNeck", "Models/head.j3o", new Mapper(0, 180, 150, 30),
				Vector3f.UNIT_Y.mult(-1), new Vector3f(0, 10, 20), 90);
		putNode("i01.head.jaw", "i01.head.rothead", "Models/jaw.j3o", new Mapper(0, 180, 0, 180),
				Vector3f.UNIT_X.mult(-1), new Vector3f(-5, 60, -50), 90);
	...
	public Jme3Object putNode(String name, String parentName, String assetPath, Mapper mapper, Vector3f rotationMask,
			Vector3f localTranslation, double currentAngle) {
		if (nodes.containsKey(name)) {
			log.error("there is already a node named {}", name);
		}
		return nodes.put(name, new Jme3Object(this, name, parentName, assetPath, mapper, rotationMask, localTranslation,
				currentAngle));
	}
