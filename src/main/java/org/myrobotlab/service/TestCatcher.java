...
public class TestCatcher extends Service implements SerialDataListener {
	...
	/**
	 * some pub/sub interfaces do not use the Message queue to post their data - but
	 * use a callback thread from the other service as an optimization onByte is one
	 * of those methods
	 */
	@Override
	...
	public BlockingQueue<Message> waitForMsgs(int count, int timeout) throws InterruptedException, IOException {
		...
		while ((interCount = msgs.size()) < count) {
			if (timeout < System.currentTimeMillis() - start) {
				throw new IOException(String.format("timeout - %d msgs under %d ms expected - got %d in %d ms",
						interCount, timeout, msgs.size(), System.currentTimeMillis() - start));
			}
			sleep(10);
		}
	...
	public ArrayList<?> waitForData(int count, int timeout, int pollInterval) throws InterruptedException, IOException {
		...
		while (msgCount < count) {
			...
			if (now - start > timeout) {
				throw new IOException(String.format("waited %d ms received %d messages expecting %d in less than %d ms",
						now - start, ret.size(), count, timeout));
			}
		}
	...
	public void checkMsg(long timeout, String method, Object... checkParms) throws InterruptedException, IOException {
		...
		if (checkParms != null && checkParms.length != msg.data.length) {
			log.error("{}", msg);
			throw new IOException(String.format("incorrect number of expected parameters - expected %d got %d",
					checkParms.length, msg.data.length));
		}
	...
	public static void main(String[] args) {
		...
		try {
			 ...
			 * 
			 * catcher01.subscribe(thrower.getName(), "throwInteger", catcher01.getName(),
			 * "catchInteger");
			 * 
			 * for (int i = 0; i < 1000; ++i) { thrower.invoke("throwInteger", i); if (i %
			 * 100 == 0) { thrower.sendBlocking(catcher01.getName(), "catchInteger"); } }
			 */
