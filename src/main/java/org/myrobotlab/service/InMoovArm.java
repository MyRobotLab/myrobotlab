...
public class InMoovArm extends Service implements IKJointAngleListener {
	...
	public String getScript(String inMoovServiceName) {
		return String.format(Locale.ENGLISH, "%s.moveArm(\"%s\",%.2f,%.2f,%.2f,%.2f)\n", inMoovServiceName, side,
				bicep.getPos(), rotate.getPos(), shoulder.getPos(), omoplate.getPos());
	}
	...
	public void setLimits(int bicepMin, int bicepMax, int rotateMin, int rotateMax, int shoulderMin, int shoulderMax,
			int omoplateMin, int omoplateMax) {
	...
	// ------------- added set pins
	/*
	 * OLD WAY public void setpins(Integer bicep, Integer rotate, Integer shoulder,
	 * Integer omoplate) {
	 * 
	 * log.info(String.format("setPins %d %d %d %d %d %d", bicep, rotate, shoulder,
	 * omoplate)); // createPeers(); this.bicep.setPin(bicep);
