...
public class Tracking extends Service {
	...
	/*
	 * public OpenCVData setLocation(OpenCVData data) { data.setX(x.getPosition());
	 * data.setY(y.getPosition()); return data; }
	 */
	...
	public void waitForObjects(OpenCVData data, float width, float height) {
		...
		if (!STATE_LEARNING_BACKGROUND.equals(state) && numberOfNewObjects == 1) {
			...
			if ((width - rect.width) / width < sizeIndexForBackgroundForegroundFlip
					&& (height - rect.height) / height < sizeIndexForBackgroundForegroundFlip) {
				learnBackground();
		...
		if (numberOfNewObjects != lastNumberOfObjects) {
			info(String.format(
					"%s - unstable change from %d to %d objects - reset clock - was stable for %d ms limit is %d ms",
					state, lastNumberOfObjects, numberOfNewObjects, System.currentTimeMillis() - lastTimestamp,
					waitInterval));
