...
public class Shoutbox extends Service {
	...
	public Shout publishShout(Shout shout) throws NotConnectedException, XMPPException {
		...
		if (xmpp != null && !TYPE_SYSTEM.equals(shout.type)) {
			for (int i = 0; i < xmppRelays.size(); ++i) {
				...
				// other activity !
				log.info("sending from {} {} to xmpp client - relayName [{}] jabberID [{}] shout.msg [{}]",
						Thread.currentThread().getId(), shout.from, relayName, jabberID, shout.msg);
	...
	public NameProvider setNameProvider(String classname)
