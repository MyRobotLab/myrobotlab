...
public class VirtualServoController extends Service implements ServoController {
	...
	class SpeedControl implements Runnable {
		...
		public void run() {
			...
			log.info("moving {} from {} -to-> {} a total of {} degrees @ {} degrees/second for {} ms", servo.getName(),
					startPos, targetPos, totalDeltaPos, velocity, totalEta);
			...
			while (isRunning) {
				...
				log.info("incremental move {} from {} to {} position at deta time {} ms is {}", servo.getName(),
						startPos, targetPos, deltaTime, pos / 1000);
	...
	public void servoMoveTo(ServoControl servo) {
		...
		sc.start();
		/*
		 * Message msg = Message.createMessage(getName(), jme.getName(), "servoMoveTo",
		 * new Object[] { servo }); jme.addMsg(msg);
		 */
		// jme.update
	}
