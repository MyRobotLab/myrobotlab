...
public class DiyServo extends Service implements ServoControl, PinListener {
						...
						if (pid.compute(pidKey)) {
							...
							if (output != lastOutput) {
								...
								if (isMoving()) {
									...
									// tolerance
									if (currentPosInput == lastCurrentPosInput
											&& Math.abs(targetPos - getCurrentPosOutput()) <= targetPosAngleTolerence) {
	 ...
	 * 
	 * @param disableDelayGrace - milliSeconds
	 * @default - 1000
	 */
	 ...
	 * Equivalent to Arduino's Servo.attach(currentPin) In this service it stops the
	 * motor and PID is set to manual mode
	 */
	@Override
	public void attach() {
		attach(pin);
	...
	/**
	 * The most important method, that tells the servo what position it should move
	 * to
	 */
