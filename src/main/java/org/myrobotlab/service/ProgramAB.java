...
public class ProgramAB extends Service implements TextListener, TextPublisher {
	...
	transient Pattern oobPattern = Pattern.compile("<oob>.*?</oob>",
			Pattern.CASE_INSENSITIVE | Pattern.DOTALL | Pattern.MULTILINE);
	transient Pattern mrlPattern = Pattern.compile("<mrl>.*?</mrl>",
			Pattern.CASE_INSENSITIVE | Pattern.DOTALL | Pattern.MULTILINE);
	...
	private String createSessionPredicateFilename(String username, String botName) {
		...
		String predicatePath = getPath() + File.separator + "bots" + File.separator + botName + File.separator
				+ "config";
	 ...
	 * @param username - the user that is sending the query
	 * @param botName  - the name of the bot you which to get the response from
	...
	private OOBPayload parseOOB(String oobPayload) {
		...
		// and params values.
		// grab the service
		Pattern servicePattern = Pattern.compile("<service>(.*?)</service>",
				Pattern.CASE_INSENSITIVE | Pattern.DOTALL | Pattern.MULTILINE);
		Matcher serviceMatcher = servicePattern.matcher(oobPayload);
		serviceMatcher.find();
		String serviceName = serviceMatcher.group(1);

		Pattern methodPattern = Pattern.compile("<method>(.*?)</method>",
				Pattern.CASE_INSENSITIVE | Pattern.DOTALL | Pattern.MULTILINE);
	 ...
	 * listener to the notify framework any service which subscribes must implement
	 * -&gt; onX(data) - this is where the data will be sent (the call-back)
	 * 
	 */
