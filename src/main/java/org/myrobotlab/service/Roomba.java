...
public class Roomba extends Service {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// roomba.pause(30);
			/*
			 * System.out.println("Checking for Roomba... "); if( roomba.updateSensors() )
			 * System.out.println("Roomba found!"); else
			 * System.out.println("No Roomba. :(  Is it turned on?");
			 * 
			 * //roomba.updateSensors();
			 * 
			 * System.out.println("Playing some notes"); roomba.playNote( 72, 10 ); // C
			 * roomba.pause( 200 ); roomba.playNote( 79, 10 ); // G roomba.pause( 200 );
			 * roomba.playNote( 76, 10 ); // E roomba.pause( 200 );
			 * 
			 * System.out.println("Spinning left, then right"); roomba.spinLeft();
			 * roomba.pause(1000); roomba.spinRight(); roomba.pause(1000); roomba.stop();
			 * 
			 * System.out.println("Going forward, then backward"); roomba.goForward();
			 * roomba.pause(1000); roomba.goBackward(); roomba.pause(1000); roomba.stop();
			 * 
			 * 
			 * System.out.println("Moving via send()"); byte cmd[] =
			 * {(byte)RoombaComm.DRIVE, (byte)0x00,(byte)0xfa, (byte)0x00,(byte)0x00};
			 * roomba.send( cmd ) ; roomba.pause(1000); roomba.stop(); cmd[1] = (byte)0xff;
			 * cmd[2] = (byte)0x05; roomba.send( cmd ) ; roomba.pause(1000); roomba.stop();
			 * 
	...
	/**
	 * Connect to a serial port specified by portid doesn't guarantee connection to
	 * Roomba, just to serial port
	 * 
	 * @param portName name of port, e.g. "/dev/cu.KeySerial1" or "COM3"
	 */
	...
	public void printSensors() {
		System.out.println(System.currentTimeMillis() + ":" + "bump:" + (roombacomm.bumpLeft() ? "l" : "_")
				+ (roombacomm.bumpRight() ? "r" : "_") + " wheel:" + (roombacomm.wheelDropLeft() ? "l" : "_")
				+ (roombacomm.wheelDropCenter() ? "c" : "_") + (roombacomm.wheelDropLeft() ? "r" : "_"));
	 ...
	 * 
	 * <pre> java roombacomm.Spiro1 /dev/cu.KeySerial1 velocity radius waittime<br>
	 * Usage: \n"+ roombacomm.Spiro1 &lt;serialportname&gt; [protocol]
	 * &lt;velocity&gt; &lt;radius&gt; &lt;waittime&gt; [options]<br> where:
	 * protocol (optional) is SCI or OI velocity and radius in mm, waittime in
	 * milliseconds [options] can be one or more of: -debug -- turn on debug output
	 * -hwhandshake -- use hardware-handshaking, for Windows Bluetooth
