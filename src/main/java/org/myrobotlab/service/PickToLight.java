...
public class PickToLight extends Service implements GpioPinListenerDigital {
	...
	/**
	 * Worker is a PickToLight level thread which operates over (potentially) all of
	 * the service modules. Displays have their own
	 * 
	 */
	...
	public void handleGpioPinDigitalStateChangeEvent(GpioPinDigitalStateChangeEvent event) {
		 ...
		 * else if (pin.getName().equals("GPIO 1")) { modules.get("02").blinkOff("ok");
		 * } else if (pin.getName().equals( "GPIO 2")) {
		 * modules.get("03").blinkOff("ok"); } else if (pin.getName().equals("GPIO 3"))
	...
	public SOAPResponse register() {
		...
		String body = String.format(soapRegisterTemplate, plant, "name", controller.getMacAddress(),
				controller.getIpAddress(), sb.toString());
	...
	public void sendEmail() {
		try {
			...
			Controller c = getController();
			String[] to = properties.getProperty("mail.smtp.to", "greg.perry@daimler.com,brett.hutton@daimler.com")
					.split(",");
			email.sendEmail(to, String.format("Hello from Controller %s %s", c.getMacAddress(), c.getIpAddress()),
					CodecUtils.toJson(c));
		} catch (Exception e) {
			Logging.logError(e);
		}
