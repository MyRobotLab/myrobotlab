...
public class Gps extends Service implements SerialDataListener {
	 ...
	 * /for-several-years-now-i-have-been.html http://stefanbangels.blogspot.be/2014
	...
	/**
	 * The FV-M8 module skips one of the latter elements in the string, leaving only
	 * 0-14 elements. GGA Global Positioning System Fixed Data GLL Geographic
	 ...
	 * DOP and Active Satellites MSS MSK Receiver Signal kmc - so the data you have
	...
	public String[] publishGGAData() {
		...
		try {
			...
			log.info(
					"GPS quality ('0' = no fix, '1' = GPS SPS fix valid, '2' = DGPS, SPS fix valid, '6' = Dead Reckoning fix valid, '8' = simulated): "
							+ tokens[6]);
			...
			log.info(
					"Geoid Separation: (Geoid-to-ellipsoid separation. Ellipsoid altitude = MSL Altitude + Geoid Separation.) "
							+ tokens[11]);
	...
	public String[] publishGSAData() {
		...
		log.info("Mode 1: ('M' = Manually forced into 2D or 3D, 'A' = Automatically allowed to switch between 2D/3D) "
				+ tokens[1]);
