...
import org.myrobotlab.service.config.JythonConfig;
...
/**
 * 
 * Jython - This service provides python scripting support. It uses the jython
 ...
 * 
 */
public class Jython extends Service implements ServiceLifeCycleListener, MessageListener {
  
  /**
   * this thread handles all callbacks to Jython process all input and sets msg
   * handles
   * 
   */
  public class InputQueue implements Runnable {
    transient protected Jython python;
    protected volatile boolean running = false;
    transient protected Thread myThread = null;

    public InputQueue(Jython python) {
      this.python = python;
    }
  ...
  public final static transient Logger log = LoggerFactory.getLogger(Jython.class);
  ...
  /**
   * Set a Jython variable with a value from Java e.g. python.set("my_var", 5)
   * 
   * @param pythonRefName
  ...
  /**
   * Get a Jython value from Jython into Java return type is PyObject wrapper
   * around the value
   * 
   * @param pythonRefName
   *          - name of variable
  ...
  /**
   * Get the value of the Jython variable e.g. Integer x =
   ...
   */
  List<String> localJythonFiles = new ArrayList<String>();
  ...
  public Jython(String n, String id) {
    super(n, id);
  ...
  public void openScript(String scriptName) throws IOException {
    JythonConfig c = (JythonConfig)config;
  ...
  /**
   * append more Jython to the current script
   * 
   * @param data
   *          the code to append
  ...
  /**
   * runs the pythonConsole.py script which creates a Jython Console object and
  ...
  public void addModulePath(String path) {
    ...
    if (interp != null) {
      ...
      log.info("Jython System Path: {}", sys.path);
    }    
  }
   ...
   * script. Jython return statement as return
   * 
   * @param code
   *          - the script to execute
  ...
  /**
   * executes an external Jython file
   * 
   * @param filename
  ...
  public List<String> getFileListing() {
    try {
      ...
      localJythonFiles = new ArrayList<String>();
      for (int i = 0; i < files.size(); ++i) {
        localJythonFiles.add(files.get(i).getName());
      }
      return localJythonFiles;
    } catch (Exception e) {
      Logging.logError(e);
    }
    return null;
  ...
  /*
   * this method can be used to load a Jython script from the Jython's local
   ...
   * Jython script. There they can be handled in the scripted users code.
   * 
  ...
  public void init() {
    ...
    localJythonFiles = getFileListing();
  ...
  public void onJython(String code) {
    log.info("onJython {}", code);
    exec(code);
  }
  ...
  public ServiceConfig apply(ServiceConfig c) {
    JythonConfig config = (JythonConfig) super.apply(c);
  ...
  public static void main(String[] args) {
    try {
      ...
      webgui.startService();
      Jython python = (Jython) Runtime.start("python", "Jython");
