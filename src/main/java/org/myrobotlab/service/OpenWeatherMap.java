...
public class OpenWeatherMap extends HttpClient {
	...
	private JSONObject getJsonFromOwm(int hourPeriod) {
		...
		JSONObject obj = null;
		try {
			apiUrl = apiForecast + URLEncoder.encode(location, "utf-8") + "&appid=" + getKey() + "&mode=json&units="
					+ units + "&lang=" + lang + "&cnt=" + hourPeriod;
	...
	/**
	 * @param location format : town,country code
	 */
	...
	/**
	 * @param period period : integer from 1 to 40 ( 1 = 3 hours )
	 */
	...
	static public ServiceType getMetaData() {
		...
		meta.addDescription(
				"This service will query OpenWeatherMap for the current weather.  Get an API key at http://openweathermap.org/");
	...
	public static void main(String[] args) {
		...
		// tomorrow is 8 ( 3 * 8 )
		owm.setUnits("metric");
		String sentence = "( Raw code : " + owm.getWeatherCode() + "), In " + owm.getLocation() + " the weather is "
				+ owm.getWeatherDescription() + ".  " + owm.getDegrees() + " degrees " + owm.getLocalUnits()
				+ " humidity " + owm.getHumidity() + " Min Degrees " + owm.getMinDegrees() + " max Degrees "
				+ owm.getMaxDegrees() + " pressure " + owm.getPressure() + " Wind Speed " + owm.getWindSpeed()
				+ " Wind Orientation " + owm.getWindOrientation();
		log.info(sentence);
	}
