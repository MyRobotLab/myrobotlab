	...
	public void play_playclick() {
		try {
			AudioInputStream audioInputStream = AudioSystem
					.getAudioInputStream(getClass().getResourceAsStream("/resource/ServoOrchestrator/click.wav"));
				...
				if (line.equals("#size")) {
					...
					if (line.equals("null")) {
						...
						sogui_ref.middlemiddle_ref.panels[counter / sizey][counter
								% sizey] = new ServoOrchestratorGUI_middlemiddle_panel(linesplit);
						final int counterf = counter;
						sogui_ref.middlemiddle_ref.panels[counter / sizey][counter % sizey].servo_goal.getDocument()
									...
									public void adjust() {
										...
										while (searchpos < sogui_ref.middlemiddle_ref.panels.length) {
											if (sogui_ref.middlemiddle_ref.panels[searchpos][i2] == null) {
												...
												sogui_ref.middlemiddle_ref.panels[searchpos][i2].servo_start.setText(
														sogui_ref.middlemiddle_ref.panels[counterf / sizey][counterf
																% sizey].servo_goal.getText() + "");
												break;
											}
										}
									}
	...
	public void top_save_button() {
		try {
			...
			for (SettingsItemHolder sih : settingsitemholder) {
				bw.write(sih.name + "~" + sih.min + "~" + sih.max + "~" + sih.startvalue + "~" + sih.arduinopos + "~"
			...
			for (ServoOrchestratorGUI_middlemiddle_panel p : sogui_ref.middlemiddle_ref.prep) {
				bw.write(p.type + "~" + p.id + "~" + p.channel_name.getText() + "~" + p.servo_start.getText() + "~"
						+ p.servo_channelid.getText() + "~" + p.servo_goal.getText() + "~" + p.servo_min.getText() + "~"
						+ p.servo_max.getText());
				bw.newLine();
			}
			bw.write("#panels");
			...
			for (ServoOrchestratorGUI_middlemiddle_panel[] p1 : sogui_ref.middlemiddle_ref.panels) {
				for (ServoOrchestratorGUI_middlemiddle_panel p : p1) {
					if (p != null) {
						bw.write(p.type + "~" + p.id + "~" + p.channel_name.getText() + "~" + p.servo_start.getText()
								+ "~" + p.servo_channelid.getText() + "~" + p.servo_goal.getText() + "~"
