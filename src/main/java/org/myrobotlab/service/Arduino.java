...
public class Arduino extends Service implements Microcontroller, PinArrayControl, I2CBusController, I2CController,
		...
		PortConnector, RecordControl, SerialRelayListener, PortListener, PortPublisher, EncoderController {
	...
	static public ServiceType getMetaData() {
		...
		meta.addDescription(
				"controls an Arduino microcontroller as a slave, which allows control of all the devices the Arduino is attached to, such as servos, motors and sensors");
	...
	/**
	 * path of the Arduino IDE must be set by user should not be static - since gson
	 * will not serialize it, and it won't be 'saved()'
	 */
	 ...
	 * if the user wants to upload first a npe will be thrown so we default it here
	 * to Uno
	 */
	String board;
	String aref;
	...
	public List<PinDefinition> getPinList() {
		...
		if (board.contains("mega")) {
			...
			if (board.contains("nano")) {
				/*
				 * int i = 20; pinName = String.format("A%d", i - 14); PinDefinition pindef =
				 * new PinDefinition(getName(), i); pindef.setDigital(false);
	...
	/**
	 * sends a heartbeat, if not replied from in the next heartbeat disconnects and
	 * resets
	 */
	// > heartbeat
	/*
	...
	public void attachI2CControl(I2CControl control) {
		...
		if (i2cDevices.containsKey(key)) {
			log.error("Device {} {} {} already exists.", control.getDeviceBus(), control.getDeviceAddress(),
					control.getName());
		} else {
	...
	/**
	 * Internal Arduino method to create an i2cBus object in MrlComm that is shared
	 * between all i2c devices
	 * 
	 * @param control
	 * @param busAddress
	 */
	...
	public int i2cRead(I2CControl control, int busAddress, int deviceAddress, byte[] buffer, int size) {
		...
		try {
			/**
			 * We will wait up to retryMax times to get the i2c data back from MrlComm.c and
			 * wait 1 ms between each try. A blocking queue is not needed, as this is only a
			 * single data element - and blocking is not necessary.
			 */
	...
	public boolean isConnected() {
		...
		if ((controllerAttachAs == MRL_IO_SERIAL_1 || controllerAttachAs == MRL_IO_SERIAL_2
				|| controllerAttachAs == MRL_IO_SERIAL_3) && boardInfo.getVersion() == MRLCOMM_VERSION) {
			return true;
		}
		return false;
	}
	...
	/**
	 * Callback for Serial service - local (not remote) although a publish/subscribe
	 * could be created - this method is called by a thread waiting on the Serial's
	 * RX BlockingQueue
	 *
	 * Other services may use the same technique or subscribe to a Serial's
	 * publishByte method
	 *
	 * it might be worthwhile to look in optimizing reads into arrays vs single byte
	 * processing .. but maybe there would be no gain
	 *
	 */
	...
	public Integer onByte(Integer newByte) {
		try {
			 ...
			 * design, but the details of how it behaves is is handled in the Serial service
			 * and we are given a unified interface
			 *
			 * The "read()" is data taken from a blocking queue in the Serial service. If we
			 * want to support blocking functions in Arduino then we'll "publish" to our
			 * local queues
			 */
			...
			if (byteCount == 2 + msgSize) {
				...
				/**
				 * acking is done in Msg ! if (boardInfo.isValid() && ackEnabled) { synchronized
				 * (ackRecievedLock) { try { long ts = System.currentTimeMillis(); log.info(
				 * "***** starting wait *****"); ackRecievedLock.wait(10000); log.info("*****
				 * waited {} ms *****", (System.currentTimeMillis() - ts)); } catch
	 ...
	 * not be part of the PinArrayControl interface - because when it comes down to
	 * it .. a pin MUST ALWAYS have an address regardless what you label or name
	 * it...
	 * 
	 */
	...
	public BoardInfo publishBoardInfo(Integer version/* byte */, Integer boardType/* byte */,
		...
		/*
		 * boolean broadcast = false; if (version != boardInfo.getVersion() || boardType
		 * != boardInfo.getBoardType()) { broadcast = true; }
		 */
	...
	public void attachMotorControl(MotorControl motor) throws Exception {
		...
		if (motor.getClass().equals(Motor.class)) {
			...
			throw new IOException(
					String.format("do not know how to attach Motor type %s", motor.getClass().getSimpleName()));
		}
	...
	public void servoSweepStart(ServoControl servo) {
		...
		log.info("servoSweep {} id {} min {} max {} step {}", servo.getName(), deviceId, servo.getMin(), servo.getMax(),
				servo.getVelocity());
	...
	/**
	 * servo.write(angle) https://www.arduino.cc/en/Reference/ServoWrite The msg to
	 * mrl will always contain microseconds - but this method will (like the Arduino
	 * Servo.write) accept both degrees or microseconds. The code is ported from
	 * Arduino's Servo.cpp
	 */
	@Override
	...
	/**
	 * On standard servos a parameter value of 1000 is fully counter-clockwise, 2000
	...
	/**
	 * send a reset to MrlComm - all devices removed, all polling is stopped and all
	 * other counters are reset
	 */
	// > softReset
	 ...
	 * write(address, value) which follows the convention of 'all' device operations
	 * at the lowest level http://codewiki.wikidot.com/c:system-calls:write
	...
	public static void main(String[] args) {
		try {
			...
			Adafruit16CServoDriver adafruit = (Adafruit16CServoDriver) Runtime.start("adafruit",
