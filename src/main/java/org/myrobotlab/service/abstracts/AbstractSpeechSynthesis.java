...
public abstract class AbstractSpeechSynthesis extends Service
		implements SpeechSynthesis, TextListener, KeyConsumer, AudioListener {
	...
	public static class Voice implements Serializable {
		...
		/**
		 * Serializable key of voice implementation - to be used to map this MRL Voice
		 * to a voice implementation
		 */
		...
		public Voice(String name, String gender, String lang, Object voiceProvider) {
			...
			if (gender != null) {
				...
				if (!"male".equals(g) && !"female".equals(g)) {
					log.warn(
							"only know about male or female - but will set gender to {} because we don't want to discriminate",
							g);
				}
				this.gender = g;
			}
	 ...
	 * some cases, only certain operating systems are supported. We are going to be
	 * pessimistic - MarySpeech is "always" ready :)
	 */
	...
	/**
	 * These methods are callback events from the AudioFile service which is a peer
	 * of this service. The speech service sends its own event based on textual
	 * data, in addition it re-broadcasts the events from audiofile which were used
	 * with this text to speak, including the sound file info
	 */
	...
	/**
	 * set the volume of the speech synthesis - default is to set the audio caching
	 * system's volume - override if necessary
	 */
	@Override
	...
	public String getLocalFileName(String toSpeak) {
		if (getVoice() != null) {
			...
			filename += getClass().getSimpleName() + File.separator + getVoice().getName() + File.separator
					+ MathUtils.md5(toSpeak) + getAudioCacheExtension();
	 ...
	 * sound files, SSML, TarsosDsp would all be prepared here before the audio data
	 * is generated
	 * 
	 * @param toSpeak
	 * @param block
	...
	public List<AudioData> parse(String toSpeak, boolean block) {
		...
		for (String speak : spokenParts) {
			...
			if (speak.startsWith("#") && speak.endsWith("#")) {
				audioData = new AudioData(System.getProperty("user.dir") + File.separator + "audioFile" + File.separator
						+ "voiceEffects" + File.separator + speak.substring(1, speak.length() - 1) + ".mp3");
			} else {
	...
	/**
	 * This method will be called by when AbstractSpeechSynthesis realizes it has no
	 * voices. Its the responsibility of the subclass to addVoice("name", "gender",
	 * "lang", impl) all the voices it provides
	 */
	 ...
	 * probably need keys and this is where the cloud provider returns the key names
	 * it needs.
	 * 
