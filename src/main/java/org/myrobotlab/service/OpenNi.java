...
// HandTracker.NewFrameListener
{
	...
	public void addRubySketchUpFrame(Skeleton skeleton, boolean singleFrame) {
		try {
			...
			sb.append(String.format("leftElbow = [%f,%f,%f]\n", skeleton.leftElbow.x, skeleton.leftElbow.z,
					skeleton.leftElbow.y));
			sb.append(String.format("leftHand = [%f,%f,%f]\n", skeleton.leftHand.x, skeleton.leftHand.z,
					skeleton.leftHand.y));
	...
	void extractSkeleton(int userId) {
		 ...
		 * Math.round(rightShoulderAngleYZ), Math.round(leftShoulderAngleYZ)), 20, 30);
		 * g2d.drawString(String.format("omoplate min %d max %d %d %d",
		 * Math.round(rightShoulderAngleXY), Math.round(leftShoulderAngleXY)), 20, 40);
		 * g2d.drawString(String.format("bicep min %d max %d %d %d",
