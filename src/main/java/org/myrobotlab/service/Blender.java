...
public class Blender extends Service {
	...
	/**
	 * Control line - JSON over TCP/IP This is the single control communication line
	 * over which virtual objects are created - and linked with Serial connections
	 * 
	...
	private transient static Gson gson = new GsonBuilder().setDateFormat("yyyy-MM-dd HH:mm:ss.SSS")
	 ...
	 * VirtualPort>();
	 * 
	 * public class VirtualPort { public Serial serial; public VirtualNullModemCable
	 * cable; }
	 */
	...
	/*
	 * call back from Blender when python script does an attach to a virtual device
	 * - returns name of the service attached
	 */
		...
		try {
			...
			if ("org.myrobotlab.service.Arduino".equals(si.getType())) {
				...
				if (arduino != null) {
					 ...
					 * Runtime.start(String.format("%s.UART.%d" ,arduino.getName(), vpn), "Serial");
					 * vp.cable = Serial.createNullModemCable(String.format("MRL.%d", vpn),
					 * String.format("BLND.%d", vpn)); virtualPorts.put(arduino.getName(), vp);
	...
	public static void main(String[] args) {
		...
		try {
			 ...
			 * 
			 * // rothead.moveTo(90); neck.moveTo(90); sleep(100); // rothead.moveTo(120);
			 * neck.moveTo(120); sleep(100); // rothead.moveTo(0); neck.moveTo(0);
			 * sleep(100); // rothead.moveTo(90); neck.moveTo(90); sleep(100); //
			 * rothead.moveTo(120); neck.moveTo(120); sleep(100); // rothead.moveTo(0);
			 * neck.moveTo(0); sleep(100);
			 * 
