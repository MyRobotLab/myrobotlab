 ...
 *         range. When a mapping is needed LegacyServo.map can be used. For this
 *         mapping LegacyServo.map(0.0, 1.0, 0, 180) might be desired. Reversing input
 *         would be done with LegacyServo.map(180, 0, 0, 180)
 * 
...
public class LegacyServo extends Service implements ServoControl {
  ...
  public final static Logger log = LoggerFactory.getLogger(LegacyServo.class);
  ...
  public void attach(Attachable attachable) throws Exception {
    ...
    if (ServoController.class.isAssignableFrom(attachable.getClass())) {
      ...
      log.info("LegacyServo {} Attaching .. targetOutput {}", getName(), targetOutput);
  ...
  static public ServiceType getMetaData() {
    ...
    ServiceType meta = new ServiceType(LegacyServo.class.getCanonicalName());
  ...
  /**
   * the 'pin' for this LegacyServo - it is Integer because it can be in a state of
   * 'not set' or null.
   * 
  ...
  /**
   * Equivalent to the Arduino IDE LegacyServo.attach(). It energizes the servo
  ...
  /**
   * Enabling PWM for the LegacyServo. Equivalent to Arduino's LegacyServo.attach(pin). It
  ...
  public void attach(String controller, String pin, double pos) throws Exception {
    log.info("LegacyServo Attach called. {} {} {}", controller, pin, pos);
  ...
  public void attach(ServoController controller, int pin, double pos) throws Exception {
    log.info("LegacyServo Controller Attach pin pos.");
    this.pin = pin;
    this.targetPos = pos;
    attach(controller);
  }
  ...
  public void sync(ServoControl sc) {
    this.addServoEventListener(this);
    ((LegacyServo) sc).addServoEventListener((NameProvider) sc);
  ...
  public void unsync(ServoControl sc) {
    ...
    ((LegacyServo) sc).removeServoEventListener((NameProvider) sc);
  ...
  public static void main(String[] args) throws InterruptedException {
    try {
      ...
      LegacyServo servo = (LegacyServo) Runtime.start("servo", "LegacyServo");
  ...
  public void saveCalibration(String calibrationFilename) {
    ...
    try {
      ...
      calibrationWriter.write("\n");
      calibrationWriter.write("# LegacyServo Config : " + this.getName() + "\n");
