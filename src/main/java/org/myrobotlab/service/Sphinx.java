...
public class Sphinx extends AbstractSpeechRecognizer {
	...
	/**
	 * Commands must be created "before" startListening startListening will create a
	 * grammar file from the data
	 *
	 */
				...
				while (isRunning) {
					...
					if (result != null) {
						...
						if (resultText.length() > 0 && isListening) {
							if (lockPhrases.size() > 0 && !lockPhrases.contains(resultText)
									&& !confirmations.containsKey(resultText)) {
							...
							if (commands != null) {
								...
								if (currentCommand != null
										&& (confirmations == null || confirmations.containsKey(resultText))) {
									...
									continue;
										...
										&& (confirmations != null || negations != null)) {
									...
									error(String.format("unknown use case for Sphinx commands - word is %s",
											resultText));
	 ...
	 * needed
	 * 
	 * example: Sphinx.createGrammar ("ear", "stop | go | left | right | back"); ear
	 * = Runtime.create("ear", "Sphinx")
	 * 
	...
	public boolean createGrammar(String grammar) {
		...
		simplexml = simplexml.replaceAll("name=\"grammarName\" value=\"simple\"",
				"name=\"grammarName\" value=\"" + grammarFileName + "\"");
		try {
	 ...
	 * that a system will not listen when its talking, otherwise a feedback loop can
	 * occur
	 * 
	 * 
	 */
	...
	/**
	 * Event is sent when the listening Service is actually listening. There is some
	 * delay when it initially loads.
	 */
	@Override
	...
	/*
	 * FIXME - the trunk is broke - the configuration is horrible find a way to make
	 * this work, despite Sphinx's chaos !
	 * 
	 * function to swap grammars to allow sphinx a little more capability regarding
	 * "new words"
	 * 
	...
	/**
	 * method to suppress recognition listening events This is important when Sphinx
	 * is listening --&gt; then Speaking, typically you don't want Sphinx to listen
	 * to its own speech, it causes a feedback loop and with Sphinx not really very
	 * accurate, it leads to weirdness -- additionally it does not recreate the
	 * speech processor - so its not as heavy handed
	 */
	@Override
	...
	/**
	 * stopRecording - it does "work", however, the speech recognition part seems to
	 * degrade when startRecording is called. I have worked around this by not
	...
	public void addMouth(SpeechSynthesis mouth) {
		...
		log.info("attached Speech service {} to Sphinx service {} with default message routes", mouth.getName(),
				getName());
