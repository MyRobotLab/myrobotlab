...
public class PortJSSC extends Port implements SerialControl, SerialPortEventListener, Serializable {
	...
	public void open() throws IOException {
		try {
			...
			throw new IOException(String.format("could not open port %s  rate %d dataBits %d stopBits %d parity %d",
					portName, rate, dataBits, stopBits, parity), e);
		}
	}
		...
		try {
			while (listening && ((newByte = read()) > -1)) {
				...
				if (stats.total % stats.interval == 0) {
					...
					log.error("===stats - dequeued total {} - {} bytes in {} ms {} Kbps", stats.total, stats.interval,
							stats.ts - stats.lastTS, 8 * stats.interval / (stats.ts - stats.lastTS));
