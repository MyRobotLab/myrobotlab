...
public abstract class Port implements Runnable, SerialControl {
	...
	public void listen(HashMap<String, SerialDataListener> listeners) {
		...
		if (readingThread == null) {
			...
			readingThread.start();
			/*
			 * - this might be a good thing .. wait until the reading thread starts - but i
			 * don't remember if JSSC works this way synchronized (lock) { lock.wait(); }
			 */
		} else {
	...
	/**
	 * reads from Ports input stream and puts it on the Serials main RX line - to be
	 * published and buffered
	 */
	@Override
	public void run() {
		...
		/*
		 * - this might be a good thing .. wait until the reading thread starts - but i
		 * don't remember if JSSC works this way synchronized(lock){ lock.notifyAll(); }
		 */
	...
	/**
	 * The way rxtxLib currently works - is it will give a -1 on a read when it has
	 * no data to give although in the specification this means end of stream - for
	 * rxtxLib this is not necessarily the end of stream. The implementation there -
	 * the thread is in rxtx - and will execute serialEvent when serial data has
	 * arrived. This might have been a design decision. The thread which calls this
	 * is in the rxtxlib - so we have it call the run() method of a non-active
	 * thread class.
	 * 
