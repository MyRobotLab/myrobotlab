...
public class AudioProcessor extends Thread {
	...
	public AudioData play(AudioData data) {
		...
		try {
			...
			if (data.getFileName() != null) {
				...
				/*
				 * fis = new FileInputStream(file); bis = new BufferedInputStream(fis); in =
			...
			if (line != null) {
				...
				while (isPlaying && (nBytesRead = din.read(buffer, 0, buffer.length)) != -1) {
					...
					/*
					 * HEE HEE .. if you want to make something sound "bad" i'm sure its clipping as
					 * 130 pushes some of the values over the high range for (int i = 0; i <
					 * data.length; ++i){ data[i] = (byte)(data[i] + 130); }
					 */
					...
					if (data.volume != null) {
						...
						if (line.isControlSupported(FloatControl.Type.MASTER_GAIN)) {
							...
							if (MathUtils
									.round(ctrl.getValue(),
											3) != MathUtils.round((float) (ctrl.getMinimum()
													+ ((double) (ctrl.getMaximum() - ctrl.getMinimum()) * data.volume)),
													3)) {
								if (data.volume <= 1.0f && data.volume >= 0) {

									ctrl.setValue((float) (ctrl.getMinimum()
											+ ((double) (ctrl.getMaximum() - ctrl.getMinimum()) * data.volume)));
	...
	public void run() {
		...
		try {
			...
			while (isRunning) {
				 ...
				 * String.format("%s:%s", queueName, currentTrackCount); Object waitForLock =
				 * audioFile.getWaitForLock(key); if (waitForLock != null) { synchronized
				 * (waitForLock) { waitForLock.notify(); } } }
				 */
	 ...
	 * 
	 * // jlp.play("NeroSoundTrax_test1_PCM_Stereo_CBR_16SS_6000Hz.wav"); AudioData
	 * data = new AudioData("aaa.mp3"); // data.volume = 120.0f; data.balance = -1;
	 * 
	 * player.play(data);
	 * 
	 * } </pre>
