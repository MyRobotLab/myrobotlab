...
public class DiyServoGui extends ServiceGui implements ActionListener {
	...
	private class MapInputSliderListener implements ChangeListener, MouseListener {
		...
		public void mouseReleased(MouseEvent e) {
			if (swingGui != null) {
				myServo.map(Double.parseDouble(minInput.getText()), Double.parseDouble(maxInput.getText()),
						Double.parseDouble(minOutput.getText()), Double.parseDouble(maxOutput.getText()));
			} else {
				log.error("can not send message myService is null");
			}
		}
		...
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				...
				if (o == updateMapButton) {
					send("map", Double.parseDouble(minInput.getText()), Double.parseDouble(maxInput.getText()),
							Double.parseDouble(minOutput.getText()), Double.parseDouble(maxOutput.getText()));
					return;
				}
		...
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				...
				if (servo.isPinArrayControlSet()) {
					...
					if ((pinArrayControlList.getSelectedItem() != null)
							&& (analogInputPinList.getSelectedItem()) != null) {
						attachListenerButton.setEnabled(true);
					} else
