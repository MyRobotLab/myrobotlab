...
public class ProgramABGui extends ServiceGui implements ActionListener {
	...
	private String[] logsClassOnly = { "org.alicebot.ab.Graphmaster", "org.alicebot.ab.MagicBooleans",
	...
	public void onText(String text) {
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				try {
					responseKit.insertHTML(responseDoc, responseDoc.getLength(), "<i><b>"
							+ botName.getSelectedItem().toString() + "</b>: " + text.replace("  ", " ").trim() + "</i>",
							0, 0, null);
				} catch (Exception e) {
	...
	public void onRequest(String text) {
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				...
				try {
					responseKit.insertHTML(responseDoc, responseDoc.getLength(),
							"<b>" + userName.getText() + "</b>: " + text, 0, 0, null);
				} catch (Exception e) {
