...
public class IntegratedMovementGui extends ServiceGui implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand().equals("moveTo")) {
			...
			ObjectPointLocation opl = (ObjectPointLocation) ((JComboBox<?>) components
					.get("itemLocation-" + but.getName())).getSelectedItem();
	...
	private JPanel createItemPanel(CollisionItem ci) {
		...
		panel.add(name);
		JLabel origin = new JLabel(String.format("(%d, %d, %d)", (int) ci.getOrigin().getX(),
				(int) ci.getOrigin().getY(), (int) ci.getOrigin().getZ()));
		panel.add(origin);
		JLabel end = new JLabel(String.format("(%d, %d, %d)", (int) ci.getEnd().getX(), (int) ci.getEnd().getY(),
				(int) ci.getEnd().getZ()));
		panel.add(end);
