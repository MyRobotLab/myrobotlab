...
public class ConnectionNodeRenderer extends JLabel implements ListCellRenderer {
	...
	public Component getListCellRendererComponent(JList list, Object value, int index, boolean isSelected,
		...
		setText(String.format("<html>%s %s %s<br/>RX %s.%s %d<br/>TX %s.%s %d</html>", data.prefix, data.protocolKey,
				data.state, data.rxName, data.rxMethod, data.rx, data.txName, data.txMethod, data.tx));// data.toString();//String.format("%s
																										// connected rx
																										// %d tx %d ",
		...
		/*
		 * if (isSelected) { setBackground(Color.lightGray); setForeground(Color.white);
