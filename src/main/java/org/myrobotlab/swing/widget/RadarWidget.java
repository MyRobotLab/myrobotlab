...
public class RadarWidget extends JPanel implements Runnable {
	...
	public void paint(Graphics g1) {
		...
		g.drawLine(sensorCenter.x, sensorCenter.y, sensorCenter.x - (int) ((maxRange / zoom) * Math.cos(rad)),
				sensorCenter.y - (int) ((maxRange / zoom) * Math.sin(rad)));
		...
		for (int x = 0; x < points.size(); x++) {
			...
			g.fillOval(sensorCenter.x - (int) ((curDist / zoom) * Math.cos(rad)),
					sensorCenter.y - (int) ((curDist / zoom) * Math.sin(rad)), 2, 2);
			// if (curDot.isOld())
			// isDead = 1;
		}
		// if (rad != 0)
