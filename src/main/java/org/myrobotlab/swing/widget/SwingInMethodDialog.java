	...
	public void actionPerformed(ActionEvent e) {
		...
		if (method != null && method.length() > 0) {
			...
			MRLListener listener = new MRLListener(myService.getSrcMethodName().split(" ")[0],
					myService.getDstServiceName(), myService.getDstMethodName().split(" ")[0] // this
			// is
			// not
			// being
			// filled
			...
			graph.insertEdge(parent, null,
					SwingGuiGui.formatMethodString(listener.topicMethod, listener.callbackMethod),
					serviceCells.get(srcService), serviceCells.get(listener.callbackName));
