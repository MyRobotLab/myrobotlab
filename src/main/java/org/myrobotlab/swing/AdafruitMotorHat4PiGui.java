...
import org.myrobotlab.service.AdafruitMotorHat4Pi;
import org.myrobotlab.service.Runtime;
import org.myrobotlab.service.SwingGui;
import org.slf4j.Logger;

public class AdafruitMotorHat4PiGui extends ServiceGui implements ActionListener {

	static final long serialVersionUID = 1L;
	public final static Logger log = LoggerFactory.getLogger(AdafruitMotorHat4PiGui.class);
	...
	AdafruitMotorHat4Pi boundService = null;

	public AdafruitMotorHat4PiGui(final String boundServiceName, final SwingGui myService) {
		...
		// attachButton);
		addTopLine(createFlowPanel("Controller", attachButton, "Controller", controllerList, "Bus", deviceBusList,
				"Address", deviceAddressList));
					...
					if (attachButton.getText().equals(attach)) {
						...
						if (index != -1) {
							swingGui.send(boundServiceName, attach, controllerList.getSelectedItem(),
									deviceBusList.getSelectedItem(), deviceAddressList.getSelectedItem());
						}
					} else {
	...
	public void onState(final AdafruitMotorHat4Pi service) {
		SwingUtilities.invokeLater(new Runnable() {
			@Override
			public void run() {
				removeListeners();
