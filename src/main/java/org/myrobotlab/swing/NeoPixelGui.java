	...
	public void actionPerformed(ActionEvent e) {
		...
		if (o == attachButton) {
			if (attachButton.getText().equals(attach)) {
				...
				if (index != -1) {
					swingGui.send(boundServiceName, attach, controller.getSelectedItem(), pinList.getSelectedItem(),
		...
		for (int i = 0; i < 25; i++) {
			if (o == setPixel[i]) {
				swingGui.send(boundServiceName, "setPixel", pixelAddress[i].getText(), pixelRed[i].getText(),
						pixelGreen[i].getText(), pixelBlue[i].getText());
			}
			if (o == sendPixel[i]) {
				swingGui.send(boundServiceName, "sendPixel", pixelAddress[i].getText(), pixelRed[i].getText(),
						pixelGreen[i].getText(), pixelBlue[i].getText());
			}
		}
		if (o == animationList) {
		...
		if (o == animStart) {
			swingGui.send(boundServiceName, "setAnimation", animationList.getSelectedItem(), animRed.getText(),
					animGreen.getText(), animBlue.getText(), animSpeed.getText());
		}
	}
