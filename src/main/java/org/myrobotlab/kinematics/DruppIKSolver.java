...
public class DruppIKSolver {
	...
	void initConstants() {
		...
		M = rl2 * ((Math.cos(RPY[1]) * Math.sin(RPY[2])) / 2 + (Math.sqrt(3)
				* (Math.cos(RPY[0]) * Math.cos(RPY[2]) + Math.sin(RPY[1]) * Math.sin(RPY[0]) * Math.sin(RPY[2]))) / 2);
		N = rl2 * ((Math.cos(RPY[1]) * Math.cos(RPY[2])) / 2 - (Math.sqrt(3)
				* (Math.cos(RPY[0]) * Math.sin(RPY[2]) - Math.cos(RPY[2]) * Math.sin(RPY[1]) * Math.sin(RPY[0]))) / 2);
		O = rl2 * (Math.sin(RPY[1]) / 2 + (Math.sqrt(3) * Math.cos(RPY[1]) * Math.sin(RPY[0])) / 2);
		P = rl2 * ((Math.cos(RPY[1]) * Math.sin(RPY[2])) / 2 - (Math.sqrt(3)
				* (Math.cos(RPY[0]) * Math.cos(RPY[2]) + Math.sin(RPY[1]) * Math.sin(RPY[0]) * Math.sin(RPY[2]))) / 2);
		R = rl2 * ((Math.cos(RPY[1]) * Math.cos(RPY[2])) / 2 + (Math.sqrt(3)
				* (Math.cos(RPY[0]) * Math.sin(RPY[2]) - Math.cos(RPY[2]) * Math.sin(RPY[1]) * Math.sin(RPY[0]))) / 2);
	...
	public double solveDown() throws Exception {
		...
		return (Math.abs(st1downsols.get(0) - (2.0f / 3) * Math.PI) < Math.abs(st1downsols.get(1) - (2.0f / 3))
				? st1downsols.get(0)
				: st1downsols.get(1));
	...
	double solveMiddle() throws Exception {
		...
		return ((Math.abs(st1middlesols.get(0) - (-2.0f / 3) * Math.PI) < Math
				.abs(st1middlesols.get(1) - (-2.0f / 3) * Math.PI)) ? st1middlesols.get(0) : st1middlesols.get(1));
	}
