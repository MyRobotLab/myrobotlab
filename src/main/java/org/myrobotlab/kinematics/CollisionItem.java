...
public class CollisionItem {
	...
	public void buildMesh(HashMap<Integer[], Map3DPoint> cloudMap2) {
		...
		for (Map3DPoint point : cloudMap2.values()) {
			vertices[i++] = new Vector3f((float) point.point.getX(), (float) point.point.getZ(),
					(float) point.point.getY());
		}
	...
	private void buildCollisionItem(HashMap<Integer[], Map3DPoint> cloudMap) {
		...
		Point[] cornerPoint = new Point[] { new Point(min[0], max[1], max[2], 0, 0, 0),
				new Point(min[0], min[1], max[2], 0, 0, 0), new Point(min[0], min[1], min[2], 0, 0, 0),
				new Point(min[0], max[1], min[2], 0, 0, 0), new Point(max[0], max[1], max[2], 0, 0, 0),
				new Point(max[0], min[1], max[2], 0, 0, 0), new Point(max[0], min[1], min[2], 0, 0, 0),
				new Point(max[0], max[1], min[2], 0, 0, 0),
				new Point(min[0], min[1], ((max[2] - min[2]) / 2) + min[2], 0, 0, 0),
				new Point(((max[0] - min[0]) / 2) + min[0], min[1], max[2], 0, 0, 0),
				new Point(max[0], min[1], ((max[2] - min[2]) / 2) + min[2], 0, 0, 0),
		...
		if (closestPointToCornerType[0] == (closestPointToCornerType[1] + 6) % 8) { // cylinder
			...
			PVector[] vector = new PVector[] { new PVector(max[0] - min[0], 0, 0), new PVector(0, max[1] - min[1], 0),
					new PVector(0, 0, max[2] - min[2]) };
			PVector[] origin = new PVector[] { new PVector(min[0], vector[1].y / 2 + min[1], vector[2].z / 2 + min[2]),
					new PVector(vector[0].x / 2 + min[0], min[1], vector[2].z / 2 + min[2]),
					new PVector(vector[0].x / 2 + min[0], vector[1].y / 2 + min[1], min[2]) };
			double[] radius = new double[] { (max[0] - min[0]) / 2, (max[1] - min[1]) / 2, (max[2] - min[2]) / 2 };
			int count[] = new int[] { 0, 0, 0 };
			for (Map3DPoint map : cloudMap.values()) {
				PVector point = new PVector((float) map.point.getX(), (float) map.point.getY(),
						(float) map.point.getZ());
	...
	public double getLength() {
		return (Math.sqrt(Math.pow(origin.getX() - end.getX(), 2) + Math.pow(origin.getY() - end.getY(), 2)
				+ Math.pow(origin.getZ() - end.getZ(), 2)));
	}
