...
public class IMEngine extends Thread implements Genetic {
	...
	public void run() {
		while (true) {
			...
			if (target != null && currentPosition.distanceTo(target) < maxDistance
					&& !AiActive(IntegratedMovement.Ai.HOLD_POSITION) && !isWaitingForServo()) {
				Point cog = service.cog.computeCoG(null);
				if (AiActive(IntegratedMovement.Ai.KEEP_BALANCE)
						&& cog.distanceTo(service.cog.getCoGTarget()) > service.cog.getMaxDistanceToCog()) {
			...
			for (int i = 0; i < maxIter; i++) {
				...
				for (int j = computeArm.getNumLinks() - 1; j >= 0; j--) {
					...
					for (int k = computeArm.getNumLinks() - 1; k >= j; k--) {
						...
						CollisionItem ci = new CollisionItem(new Point(jp[k][0], jp[k][1], jp[k][2], 0, 0, 0),
								new Point(jp[k + 1][0], jp[k + 1][1], jp[k + 1][2], 0, 0, 0),
	...
	private boolean moveToGoal(Point goal, String lastDHLink) {
		...
		while (true) {
			...
			if (numSteps >= maxIterations) {
				...
				calcFitnessType = CalcFitnessType.POSITION;
				GeneticAlgorithm GA = new GeneticAlgorithm(this, geneticPoolSize, arm.getNumLinks(), 12,
						geneticRecombinationRate, geneticMutationRate);
				Chromosome bestFit = GA.doGeneration(geneticGeneration); // this is the
																			// number of
																			// time the
																			// chromosome
	...
	private Point checkCollision(DHRobotArm arm, CollisionDectection cd) {
		...
		while (time <= 2.0) {
			...
			for (int i = 0; i < checkArm.getNumLinks(); i++) {
				CollisionItem ci = new CollisionItem(new Point(jp[i][0], jp[i][1], jp[i][2], 0, 0, 0),
						new Point(jp[i + 1][0], jp[i + 1][1], jp[i + 1][2], 0, 0, 0), checkArm.getLink(i).getName());
		...
		if (collisionResult.haveCollision) {
			...
			Point vCollItem = collisionResult.collisionPoints[itemIndex]
					.subtract(collisionResult.collisionPoints[1 - itemIndex]);
			...
			if (collisionResult.collisionLocation[1 - itemIndex] > 0.0
					|| collisionResult.collisionLocation[1 - itemIndex] < 1.0) { // collision
																					// on
																					// the
																					// side
																					// of
		...
		for (Chromosome chromosome : chromosomes) {
			...
			for (int i = 0; i < computeArm.getNumLinks(); i++) {
				...
				if (chromosome.getDecodedGenome().get(i) != null) {
					...
					double delta = computeArm.getLink(i).getPositionValueDeg()
							- (double) chromosome.getDecodedGenome().get(i);
			...
			if (calcFitnessType == CalcFitnessType.POSITION) {
				...
				for (int i = 0; i < checkArm.getNumLinks(); i++) {
					if (chromosome.getDecodedGenome().get(i) != null) {
						...
						double degrees = link.getPositionValueDeg();
						double deltaDegree = java.lang.Math
								.abs(degrees - (double) chromosome.getDecodedGenome().get(i));
				...
				for (int i = 0; i < checkArm.getNumLinks(); i++) {
					CollisionItem ci = new CollisionItem(new Point(jp[i][0], jp[i][1], jp[i][2], 0, 0, 0),
							new Point(jp[i + 1][0], jp[i + 1][1], jp[i + 1][2], 0, 0, 0),
	...
	private Point moveToObject() {
		...
		case LEFT_SIDE:
		case RIGHT_SIDE: {
			point = service.collisionItems.getClosestPoint(moveInfo.targetItem, lastLinkItem, new Double[] { 0.5, 0.5 },
					vector);
			addRadius = true;
		}
		case CENTER: {
		...
		if (addRadius) {
			...
			Point side0 = new Point(point[0].getX() + vectorT[0], point[0].getY() + vectorT[1],
					point[0].getZ() + vectorT[2], 0, 0, 0);
			Point v = new Point(vectorT[0], vectorT[1], vectorT[2], 0, 0, 0);
			v = v.unitVector(safety);
			side0 = side0.add(v);
			Point pointF = side0;
			Point curPos = arm.getPalmPosition(moveInfo.lastLinkName);
			double d = Math.pow((side0.getX() - curPos.getX()), 2) + Math.pow((side0.getY() - curPos.getY()), 2)
					+ Math.pow((side0.getZ() - curPos.getZ()), 2);
			double currentx = side0.getX();
			for (int i = 0; i < 360; i += 10) {
				double L = vectori[0] * vectori[0] + vectori[1] * vectori[1] + vectori[2] * vectori[2];
				double x = ((moveInfo.targetItem.getOrigin().getX()
						* (Math.pow(vectori[1], 2) + Math.pow(vectori[2], 2))
						- vectori[0] * (moveInfo.targetItem.getOrigin().getY() * vectori[1]
								+ moveInfo.targetItem.getOrigin().getZ() * vectori[2] - vectori[0] * side0.getX()
								- vectori[1] * side0.getY() - vectori[2] * side0.getZ()))
						* (1 - Math.cos(MathUtils.degToRad(i)))
						+ L * side0.getX() * Math.cos(MathUtils.degToRad(i))
						+ Math.sqrt(L)
								* (-moveInfo.targetItem.getOrigin().getZ() * vectori[1]
										+ moveInfo.targetItem.getOrigin().getY() * vectori[2]
										- vectori[2] * side0.getY() + vectori[1] * side0.getZ())
								* Math.sin(MathUtils.degToRad(i)))
						/ L;
				double y = ((moveInfo.targetItem.getOrigin().getY()
						* (Math.pow(vectori[0], 2) + Math.pow(vectori[2], 2))
						- vectori[1] * (moveInfo.targetItem.getOrigin().getX() * vectori[0]
								+ moveInfo.targetItem.getOrigin().getZ() * vectori[2] - vectori[0] * side0.getX()
								- vectori[1] * side0.getY() - vectori[2] * side0.getZ()))
						* (1 - Math.cos(MathUtils.degToRad(i)))
						+ L * side0.getY() * Math.cos(MathUtils.degToRad(i))
						+ Math.sqrt(L)
								* (moveInfo.targetItem.getOrigin().getZ() * vectori[0]
										- moveInfo.targetItem.getOrigin().getX() * vectori[2]
										+ vectori[2] * side0.getX() - vectori[0] * side0.getZ())
								* Math.sin(MathUtils.degToRad(i)))
						/ L;
				double z = ((moveInfo.targetItem.getOrigin().getZ()
						* (Math.pow(vectori[0], 2) + Math.pow(vectori[1], 2))
						- vectori[2] * (moveInfo.targetItem.getOrigin().getX() * vectori[0]
								+ moveInfo.targetItem.getOrigin().getY() * vectori[1] - vectori[0] * side0.getX()
								- vectori[1] * side0.getY() - vectori[2] * side0.getZ()))
						* (1 - Math.cos(MathUtils.degToRad(i)))
						+ L * side0.getZ() * Math.cos(MathUtils.degToRad(i))
						+ Math.sqrt(L)
								* (-moveInfo.targetItem.getOrigin().getY() * vectori[0]
										+ moveInfo.targetItem.getOrigin().getX() * vectori[1]
										- vectori[1] * side0.getX() + vectori[0] * side0.getY())
								* Math.sin(MathUtils.degToRad(i)))
						/ L;
				Point check = new Point(x, y, z, 0, 0, 0);
				double dt = Math.pow((check.getX() - curPos.getX()), 2) + Math.pow((check.getY() - curPos.getY()), 2)
						+ Math.pow((check.getZ() - curPos.getZ()), 2);
