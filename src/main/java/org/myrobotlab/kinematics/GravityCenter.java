...
public class GravityCenter extends Thread {
	 ...
	 * @param name         name
	 * @param mass         mass
	 * @param centerOfMass (0.0 - 1.0) representing where the center of mass is
	 *                     located, from the origin point. If you don't know, it's
	 *                     safe to put 0.5
	 */
	...
	public synchronized Point computeCoG(CollisionDectection cd) {
		...
		for (CollisionItem ci : cd.getItems().values()) {
			if (cogs.containsKey(ci.getName())) {
				Point icog = ci.getEnd().subtract(ci.getOrigin()).unitVector(1).multiplyXYZ(cogs.get(ci.getName()))
						.multiplyXYZ(ci.getLength()).add(ci.getOrigin());
