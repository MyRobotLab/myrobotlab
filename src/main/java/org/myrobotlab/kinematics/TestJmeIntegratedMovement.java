...
public class TestJmeIntegratedMovement extends SimpleApplication {
	...
	public void simpleInitApp() {
		...
		for (CollisionItem ci : objects.values()) {
			Vector3f ori = new Vector3f((float) ci.getOrigin().getX() / 2, (float) ci.getOrigin().getZ() / 2,
					(float) ci.getOrigin().getY() / 2);
			Vector3f end = new Vector3f((float) ci.getEnd().getX() / 2, (float) ci.getEnd().getZ() / 2,
					(float) ci.getEnd().getY() / 2);
			if (!ci.fromKinect) {
				...
				for (Map3DPoint p : ci.cloudMap.values()) {
					...
					Vector3f pos = new Vector3f((float) p.point.getX() / 2, (float) p.point.getZ() / 2,
							(float) p.point.getY() / 2);
