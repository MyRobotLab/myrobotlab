...
public class TestJmeIMModel extends SimpleApplication implements IntegratedMovementInterface {
	 ...
	 * @param hookTo           : attach this part to the hook part (null to hook to
	 *                         the root)
	 ...
	 */
	public void addPart(String name, String modelPath, float modelScale, String hookTo, Vector3f relativePosition,
			Vector3f rotationMask, float initialAngle) {
	...
	public void simpleUpdate(float tpf) {
		...
		while (eventQueue.size() > 0) {
			...
			if (nodes.containsKey(event.name)) {
				...
				Vector3f rotMask = new Vector3f((float) node.getUserData("rotationMask_x"),
						(float) node.getUserData("rotationMask_y"), (float) node.getUserData("rotationMask_z"));
	...
	public void addObject(CollisionItem item) {
		...
		if (item.isFromKinect()) {
			...
			Vector3f ori = new Vector3f((float) item.getOrigin().getX(), (float) item.getOrigin().getZ(),
					(float) item.getOrigin().getY());
			Vector3f end = new Vector3f((float) item.getEnd().getX(), (float) item.getEnd().getZ(),
					(float) item.getEnd().getY());
