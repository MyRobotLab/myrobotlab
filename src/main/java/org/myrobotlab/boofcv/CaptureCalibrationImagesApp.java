			...
			if (targetTime < System.currentTimeMillis()) {
				...
				while (true) {
					if (savedImages && userChoice != -1) {
						if (userChoice == 1) {
							UtilImageIO.savePPM(savedRgb, String.format(directory + "rgb%07d.ppm", frameNumber),
									buffer);
							UtilOpenKinect.saveDepth(savedDepth,
									String.format(directory + "depth%07d.depth", frameNumber), buffer);
							frameNumber++;
							text = "Image Saved!";
						} else {
