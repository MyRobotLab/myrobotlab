...
angular.module("mrlapp.mrl", []).provider("mrl", [
  function () {
    ...
    this.request = {
      url:
        document.location.origin.toString() + "/api/messages?user=root&pwd=pwd&session_id=2309adf3dlkdk&id=" + this.id,
      transport: "websocket",
      maxRequest: 100,
    ...
    */
    ...
    this.$get = function ($q, $log, $http, $templateCache, $ocLazyLoad) {
      ...
      _self.getProperties = function (service) {
        ...
        let info = {
          autoDisable:
            "servo will de-energize if no activity occurs in {idleTimeout} ms - saving the servo from unnecessary wear or damage",
          idleTimeout: "number of milliseconds the servo will de-energize if no activity has occurred",
          isSweeping:
            "servo is in sweep mode - which will make the servo swing back and forth at current speed between min and max values",
      ...
      _self.addService = function (service) {
        ...
        $ocLazyLoad.load("service/js/" + type + "Gui.js").then(
          function () {
            console.debug("lazy-loading successful:", name, type)
            $http.get("service/views/" + type + "Gui.html").then(
              function (response) {
      ...
      let jsRuntime = {
        ...
        jvmArgs: [
          "-agentlib:jdwp=transport=dt_socket,suspend=y,address=localhost:37551",
          "-javaagent:/lhome/grperry/Downloads/eclipse-jee-2019-06-R-linux-gtk-x86_64/eclipse/configuration/org.eclipse.osgi/405/0/.cp/lib/javaagent-shaded.jar",
